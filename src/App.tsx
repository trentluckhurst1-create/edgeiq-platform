...
import TrackingTab from "./terminal/tabs/TrackingTab";
...
import { getMeetingDisplayState, isFutureMeetingDisplayState } from "./utils/meetingDisplayState";




type EdgeiqRuntimeBoundaryProps = {
  children: React.ReactNode;
};

type EdgeiqRuntimeBoundaryState = {
  errorMessage: string;
};

class EdgeiqRuntimeBoundary extends React.Component<EdgeiqRuntimeBoundaryProps, EdgeiqRuntimeBoundaryState> {
  state: EdgeiqRuntimeBoundaryState = { errorMessage: "" };

  static getDerivedStateFromError(error: unknown): EdgeiqRuntimeBoundaryState {
    return { errorMessage: error instanceof Error ? error.message : String(error) };
  }

  componentDidCatch(error: unknown): void {
    console.error("EDGEiQ runtime guard", error);
  }

  render(): React.ReactNode {
    if (this.state.errorMessage) {
      return (
        <div className="min-h-[420px] rounded-2xl border border-red-400/30 bg-[#0f1724] p-6 text-white">
          <div className="text-[11px] font-bold uppercase tracking-[0.18em] text-red-200">EDGEiQ runtime guard</div>
          <div className="mt-3 text-[18px] font-semibold">EDGEiQ runtime guard: {this.state.errorMessage}</div>
          <div className="mt-2 text-[13px] leading-6 text-slate-300">The terminal caught a render error instead of showing a blank screen.</div>
        </div>
      );
    }
...
export type RatingDisplayRow = {
  ...
  v8CandidatePrice?: number | null;
  v8CandidateDelta?: number | null;
  v8CandidateBadge?: string;
  v8CandidateStatus?: string;
  v8BrcMatchLevel?: string;
  betQualityScore?: number | null;
  betQualityGrade?: string;
  betQualityBand?: string;
  betQualityStatus?: string;
  ...
  trainer_factor_band_v1?: string;
  jockey_factor_band_v1?: string;
  combo_factor_band_v1?: string;
  trainer_jockey_blend_band_v3?: string;
  trainer_jockey_blend_score_v3?: number | null;
  tj_factor_verdict_v3?: string;
  tj_edge_label_v3?: string;
};
...
function raceReliabilityToneClass(v: unknown): string {
  const x = text(v).toUpperCase();
  if (x === "ELITE") return "reliability-elite";
  if (x === "POSITIVE") return "reliability-positive";
  if (x === "NEUTRAL") return "reliability-neutral";
  if (x === "NEGATIVE") return "reliability-negative";
  if (x === "POOR") return "reliability-poor";
  return "reliability-unknown";
}

function meetingDayBucketLabel(v: unknown): string {
  const value = text(v).toUpperCase();
  if (value === "TODAY") return "TODAY";
  if (value === "TOMORROW") return "TOMORROW";
  if (value === "DAY+2") return "DAY +2";
  return value || "UPCOMING";
}

function meetingDayBucketOrder(v: unknown): number {
  const value = text(v).toUpperCase();
  if (value === "TODAY") return 0;
  if (value === "TOMORROW") return 1;
  if (value === "DAY+2") return 2;
  return 9;
}

function meetingStatusLabel(v: unknown): string {
  const value = text(v).toUpperCase();
  if (value === "FIELDS_READY") return "FIELDS READY";
  if (value === "FIELDS_PENDING") return "FIELDS PENDING";
  if (value === "FIELDS_UNAVAILABLE") return "FIELDS UNAVAILABLE";
  return value.replace(/_/g, " ") || "UNKNOWN";
}

function meetingStatusToneClass(v: unknown): string {
  const value = text(v).toUpperCase();
  if (value === "FIELDS_READY") return "good";
  if (value === "FIELDS_PENDING") return "warn";
  if (value === "FIELDS_UNAVAILABLE") return "bad";
  return "neutral";
}

function inferredMeetingDayBucket(dateValue: unknown): string {
...
export default function App(): React.ReactElement {
  ...
  const [meetingCalendarRows, setMeetingCalendarRows] = useState<CsvRow[]>([]);
  const [meetingDiagnosticRows, setMeetingDiagnosticRows] = useState<CsvRow[]>([]);
  ...
  const [v8CandidateRows, setV8CandidateRows] = useState<CsvRow[]>([]); // V8 OBSERVATION ONLY
  const [liveBetQualityRows, setLiveBetQualityRows] = useState<CsvRow[]>([]); // BET QUALITY OBSERVATION ONLY
  ...
  const [raceReliabilityRows, setRaceReliabilityRows] = useState<CsvRow[]>([]);
  const [trainerJockeyFactorRows, setTrainerJockeyFactorRows] = useState<CsvRow[]>([]);
  ...
  useEffect(() => {
    ...
    async function run(): Promise<void> {
      try {
        setLoading(true);
        setError("");
        const [meetingCalendar, meetingDiagnostics, raceCard, raceFields, executionEngine, rated, v8Candidate, liveBetQuality, finalRatings, history, summary, speed, silks, careerStats, fullCareer, scratchings, results, standards, raceRatingTargets, bias, raceReliability, trainerJockeyFactors, pacePressure, raceShape, sectionalTempo, activeSelector] = await Promise.all([
          loadCsvOptional("/data/edgeiq_vic_three_day_meeting_calendar_v1.csv"),
          loadCsvOptional("/data/edgeiq_vic_three_day_meeting_diagnostics.csv"),
          ...
          loadCsvOptional(EDGEIQ_LIVE_FILES.terminalFeed),
          loadCsvOptional("/data/edgeiq_live_v8_candidate_display_feed_v1.csv"),
        loadCsvOptional("/data/edgeiq_live_bet_quality_v1_1.csv"),
          ...
          loadCsvOptional(EDGEIQ_LIVE_FILES.raceReliability),
          loadCsvOptional(EDGEIQ_LIVE_FILES.trainerJockeyFactors),
        ...
        if (!active) return;
        setRaceCardRows(raceCard.length ? raceCard : raceFields.length ? raceFields : executionEngine.length ? executionEngine : rated);
        setMeetingCalendarRows(meetingCalendar);
        setMeetingDiagnosticRows(meetingDiagnostics);
        setRaceFieldRows(raceFields);
        setExecutionEngineRows(executionEngine);
        setRatedRows(rated);
        setV8CandidateRows(v8Candidate);
        setLiveBetQualityRows(liveBetQuality);
        ...
        setRaceReliabilityRows(raceReliability);
        setTrainerJockeyFactorRows(trainerJockeyFactors);
...
const silkByHorse = useMemo(() => {
  ...
  const v8CandidateByRunner = useMemo(() => {
    const map = new Map<string, CsvRow>();

    for (const row of v8CandidateRows) {
      const raceDate = text(row.race_date);
      const track = text(row.track);
      const raceNo = num(row.race_no) ?? 0;
      const horse = text(row.horse);
      ...
      const key = runnerKeyOf(raceDate, track, raceNo, horse, horseKey);
      const raceKey = raceKeyOf(raceDate, track, raceNo);

      map.set(key, row);
      map.set(`${raceKey}|${compactKey(horse)}`, row);
      map.set(`${raceKey}|${horseKey}`, row);
    }

    return map;
  }, [v8CandidateRows]);
  const liveBetQualityByRunner = useMemo(() => {
    const map = new Map<string, CsvRow>();

    for (const row of liveBetQualityRows) {
      const track = String(row.track ?? "").trim().toUpperCase();
      const raceNo = String(row.race_no ?? row.raceNo ?? "").trim();
      const horse = String(row.horse ?? row.runner ?? "").trim().toUpperCase().replace(/[^A-Z0-9]/g, "");
      if (!track || !raceNo || !horse) continue;
      map.set(`${track}|${raceNo}|${horse}`, row);
    }
  ...
  const trainerJockeyByRunner = useMemo(() => {
    const map = new Map<string, CsvRow>();

    for (const row of trainerJockeyFactorRows) {
      const raceDate = text(row.race_date || row.meeting_date || row.date);
      const track = text(row.track);
      const raceNo = num(row.race_no) ?? num(row.race_number) ?? 0;
      const horse = text(row.horse);
      const horseKey = compactKey(row.horse_canon) || compactKey(row.horse_key) || compactKey(horse);

      if (!raceDate || !track || !raceNo || !horseKey) continue;

      const runnerKey = runnerKeyOf(raceDate, track, raceNo, horse, horseKey);
      const raceKey = raceKeyOf(raceDate, track, raceNo);
      map.set(runnerKey, row);
      map.set(`${raceKey}|${compactKey(horse)}`, row);
      map.set(`${raceKey}|${horseKey}`, row);
    }

    return map;
  }, [trainerJockeyFactorRows]);
  ...
  const ratingRows = useMemo<RatingDisplayRow[]>(() => {
    const rows = raceCardRows
      ...
      .map((row, index) => {
        ...
        const tjFactor =
          trainerJockeyByRunner.get(runnerKeyOf(raceDate, track, raceNo, horse, horseKey)) ??
          trainerJockeyByRunner.get(`${raceKeyOf(raceDate, track, raceNo)}|${matchKey}`) ??
          trainerJockeyByRunner.get(`${raceKeyOf(raceDate, track, raceNo)}|${horseKey}`);
        ...
        const v8Candidate =
          v8CandidateByRunner.get(runnerKeyOf(raceDate, track, raceNo, horse, horseKey)) ??
          v8CandidateByRunner.get(`${raceKeyOf(raceDate, track, raceNo)}|${compactKey(horse)}`) ??
          v8CandidateByRunner.get(`${raceKeyOf(raceDate, track, raceNo)}|${horseKey}`);

        const ratedPrice = firstPrice(num(rated?.fair_price), usablePrice(execution, "fair_price"), usablePrice(rated, "rated_price"), num(row.rated_price));
        ...
        return {
          ...
          railPosition: text(mergedRow.rail_position) || text(row.rail_position),
          todayRating:
          ...
          ratedPrice: firstPrice(num(rated?.fair_price), num(execution?.fair_price), num(mergedRow.fair_price), num(mergedRow.ui_fair_price), num(finalRating?.elite_rated_price), ratedPrice),
          marketPrice,
          marketStatus,
          marketNote,
          highEdgeReview,
          ...
          v8CandidatePrice: num(v8Candidate?.v8_candidate_price_display) ?? num(v8Candidate?.v8_interaction_candidate_price),
          v8CandidateDelta: num(v8Candidate?.v8_delta_display) ?? num(v8Candidate?.v8_interaction_price_delta),
          v8CandidateBadge: text(v8Candidate?.v8_candidate_badge),
          v8CandidateStatus: text(v8Candidate?.v8_candidate_display_status),
          v8BrcMatchLevel: text(v8Candidate?.brc_match_level_v8),
          ...
          trainer_factor_band_v1: text(tjFactor?.trainer_factor_band_v1),
          jockey_factor_band_v1: text(tjFactor?.jockey_factor_band_v1),
          combo_factor_band_v1: text(tjFactor?.combo_factor_band_v1),
          trainer_jockey_blend_band_v3: text(tjFactor?.trainer_jockey_blend_band_v3),
          trainer_jockey_blend_score_v3: num(tjFactor?.trainer_jockey_blend_score_v3),
          tj_factor_verdict_v3: text(tjFactor?.tj_factor_verdict_v3),
          tj_edge_label_v3: text(tjFactor?.tj_edge_label_v3),
  ...
  }, [raceCardRows, activeSelectorRows, activeRaceMetaByRace, ratedByRunner, finalRatingsByRunner, executionByRunner, trainerJockeyByRunner, sectionalTempoByRunner, historyByHorse, summaryByHorse, silkByHorse, scratchMap, fieldScratchMap]);
  ...
  const meetingDiagnosticsByKey = useMemo(() => {
    const map = new Map<string, CsvRow>();
    for (const row of meetingDiagnosticRows) {
      if (text(row.record_type).toUpperCase() !== "MEETING") continue;
      const raceDate = text(row.race_date);
      const track = text(row.track);
      if (!raceDate || !track) continue;
      map.set(meetingKeyOf(raceDate, track), row);
    }
    return map;
  }, [meetingDiagnosticRows]);

  const meetings = useMemo<MeetingGroup[]>(() => {
    const grouped = new Map<string, MeetingGroup>();

    const ensureMeeting = (raceDate: string, track: string, dayBucket: string, meetingType: string, fallbackStatus: string, fallbackReady: string): MeetingGroup => {
      const meetingKey = meetingKeyOf(raceDate, track);
      if (!grouped.has(meetingKey)) {
        const diagnostic = meetingDiagnosticsByKey.get(meetingKey);
        grouped.set(meetingKey, {
          meetingKey,
          raceDate,
          track,
          dayBucket: dayBucket || inferredMeetingDayBucket(raceDate),
          meetingType,
          meetingStatus: text(diagnostic?.meeting_status) || fallbackStatus,
          dashboardReady: text(diagnostic?.dashboard_ready) || fallbackReady,
          fieldRows: num(diagnostic?.field_rows) ?? 0,
          fieldRaces: num(diagnostic?.field_races) ?? 0,
          pricedRows: num(diagnostic?.priced_rows) ?? 0,
          races: [],
        });
      }
      return grouped.get(meetingKey)!;
    };

    for (const row of meetingCalendarRows) {
      const raceDate = text(row.race_date);
      const track = text(row.track);
      if (!raceDate || !track) continue;
      ensureMeeting(raceDate, track, text(row.day_bucket), text(row.meeting_type), "FIELDS_PENDING", "NO");
    }

    for (const race of raceGroups) {
      if (!race.raceDate || !race.track || !race.raceNo) continue;
      const meeting = ensureMeeting(
        race.raceDate,
        race.track,
        inferredMeetingDayBucket(race.raceDate),
        "",
        "FIELDS_READY",
        "YES",
      );
      const existing = meeting.races.find((item) => item.key === race.key || item.raceNo === race.raceNo);
      if (existing) {
        existing.raceTime = existing.raceTime || race.raceTime;
        existing.raceClass = existing.raceClass === "Class unknown" ? race.raceClass : existing.raceClass;
        existing.distance = existing.distance || race.distance;
        existing.todayTrackCondition = existing.todayTrackCondition || race.todayTrackCondition;
        existing.rows = race.rows.length ? race.rows : existing.rows;
      } else {
        meeting.races.push(race);
      }
    }

    const stubSourceRows =
      raceCardRows.length
        ? raceCardRows
        : raceFieldRows.length
          ? raceFieldRows
          : executionEngineRows;

    for (const row of stubSourceRows) {
      ...
      const meeting = ensureMeeting(
        ...
        track,
        text(row.day_bucket),
        text(row.meeting_type),
        raceDate ? "FIELDS_READY" : "FIELDS_PENDING",
        raceNo ? "YES" : "NO",
      );
      const meetingKey = text(row.meeting_key) || meeting.meetingKey;
      const raceKey = text(row.race_key) || `${meetingKey}_R${raceNo}`;
      const existing = meeting.races.find((item) => item.key === raceKey || item.raceNo === raceNo);
      ...
      if (!existing) {
        ...
        existing.raceTime = existing.raceTime || text(row.race_time) || text(row.raceTime);
        existing.raceClass = existing.raceClass === "Class unknown" ? text(row.race_class) || text(row.raceClass) || "Class unknown" : existing.raceClass;
        existing.distance = existing.distance || num(row.distance);
        existing.todayTrackCondition = existing.todayTrackCondition || text(row.track_condition) || text(row.todayTrackCondition);
      }
    }
    ...
    return [...grouped.values()]
      .map((meeting) => ({
        ...meeting,
        races: [...meeting.races]
          .filter((race) => race.raceNo > 0 && !!race.key)
          .sort(
            (a, b) =>
      ...
      .sort(
        (a, b) =>
          meetingDayBucketOrder(a.dayBucket) - meetingDayBucketOrder(b.dayBucket) ||
          (a.races[0] && b.races[0] ? selectorSortValue(a.races[0]) - selectorSortValue(b.races[0]) : 0) ||
          a.raceDate.localeCompare(b.raceDate) ||
          a.track.localeCompare(b.track)
      );
  }, [meetingCalendarRows, meetingDiagnosticsByKey, raceGroups, raceCardRows, raceFieldRows, executionEngineRows]);
  ...
  useEffect(() => {
    ...
    const selectedMeeting = meetings.find((meeting) => meeting.meetingKey === selectedMeetingKey) ?? null;
    const selectedMeetingRaces = (selectedMeeting?.races ?? []).filter((race) => race.raceNo > 0 && !!race.key);
    const selectedRaceExists = selectedMeetingRaces.some((race) => race.key === selectedRaceKey);
    ...
    const fallbackMeeting =
      ...
      meetings[0];
    const fallbackMeetingRaces = (fallbackMeeting?.races ?? []).filter((race) => race.raceNo > 0 && !!race.key);

    const fallbackRace =
      fallbackMeetingRaces.find((race) => race.raceNo === focusRaceNo) ??
      fallbackMeetingRaces[0] ??
      null;

    const shouldPreferFreshFocus =
      !initialSelectionDone.current &&
      !!fallbackMeeting &&
      fallbackMeetingRaces.length > 0 &&
      !!selectedMeeting &&
      (
        !selectedMeetingRaces.length ||
        selectedMeeting.raceDate !== fallbackMeeting.raceDate ||
        cleanTrack(selectedMeeting.track) !== cleanTrack(fallbackMeeting.track)
      );

    if (selectedMeetingExists && selectedMeeting && !selectedMeetingRaces.length && !shouldPreferFreshFocus) {
      if (selectedRaceKey) {
        setSelectedRaceKey("");
      }
    ...
    if (selectedMeetingExists && selectedRaceExists && !shouldPreferFreshFocus) {
      initialSelectionDone.current = true;
      return;
    }

    if (!selectedMeetingExists || shouldPreferFreshFocus) {
      setSelectedMeetingKey(fallbackMeeting.meetingKey);
      setSelectedHorseKey("");
    }

    if (!selectedRaceExists || shouldPreferFreshFocus) {
  ...
  useEffect(() => {
    ...
    const validRaces = currentMeeting.races.filter((race) => race.raceNo > 0 && !!race.key);
    const raceStillExists = validRaces.some((race) => race.key === selectedRaceKey);
    if (validRaces.length && (!selectedRaceKey || !raceStillExists)) {
      setSelectedRaceKey(validRaces[0]?.key ?? "");
    }
  ...
  const currentRace = useMemo(() => {
    const validRaces = (currentMeeting?.races ?? []).filter((race) => race.raceNo > 0 && !!race.key);
    return (
      validRaces.find((race) => race.key === selectedRaceKey) ??
      validRaces[0] ??
      null
    );
  }, [currentMeeting, selectedRaceKey]);

  const currentMeetingRaces = useMemo(() => {
  ...
  const upcomingMeetings = useMemo(
    () =>
      meetings.map((meeting) => ({
        ...
        track: meeting.track,
        dayBucket: meeting.dayBucket,
        meetingType: meeting.meetingType,
        meetingStatus: meeting.meetingStatus,
        dashboardReady: meeting.dashboardReady,
        fieldRows: meeting.fieldRows,
        fieldRaces: meeting.fieldRaces,
        pricedRows: meeting.pricedRows,
        raceCount: meeting.races.length || meeting.fieldRaces,
      })),
    [meetings]
  );

  const currentMeetingDisplayState = useMemo(
    () => getMeetingDisplayState(currentMeeting),
    [currentMeeting]
  );
  const currentMeetingIsFuture = isFutureMeetingDisplayState(currentMeetingDisplayState);

  const currentRaceReliability = useMemo(() => {
    if (!currentRace) return null;
    return (
      raceReliabilityRows.find((row) =>
        text(row.meeting_date || row.race_date || row.date) === currentRace.raceDate &&
        cleanTrack(row.track || row.meeting_name || row.venue) === cleanTrack(currentRace.track) &&
        (num(row.race_no ?? row.race_number ?? row.race) ?? 0) === currentRace.raceNo
      ) ?? null
    );
  }, [raceReliabilityRows, currentRace]);
...
return rows;
  ...
  const marketStateLabel = !currentMeeting
    ? "NO FEED"
    : currentMeetingIsFuture
      ? meetingStatusLabel(currentMeeting.meetingStatus)
      : !currentRace
        ? "NO FEED"
        : staleSnapshot
          ? "STALE"
          : raceStatusLabel(currentRace);
  const marketStateContext = !currentMeeting
    ? "no selected meeting"
    : currentMeetingDisplayState === "FUTURE_MEETING_WITH_FIELDS"
      ? "future meeting fields loaded"
      : currentMeetingDisplayState === "FUTURE_MEETING_WITHOUT_FIELDS"
        ? "future meeting being monitored"
      : !currentRace
          ? "no selected race"
          : staleSnapshot
  ...
  const currentMeetingStatusLabel = currentMeeting ? meetingStatusLabel(currentMeeting.meetingStatus) : "UNKNOWN";
  const currentMeetingDayBucketLabel = currentMeeting ? meetingDayBucketLabel(currentMeeting.dayBucket) : "";
  const currentMeetingRaceCount = currentMeeting ? (currentMeeting.races.length || currentMeeting.fieldRaces || 0) : 0;
  const currentMeetingSummaryText = currentMeeting
    ? `${displayDate(currentMeeting.raceDate)} | ${currentMeeting.track}`
    : "";

  const marketStateTone = currentMeeting ? meetingStatusToneClass(currentMeeting.meetingStatus) : "neutral";
  const meetingNavigatorSummary = currentMeetingDisplayState === "FUTURE_MEETING_WITH_FIELDS"
    ? "Fields are loaded for this future meeting. Market prices, track profile and race intelligence will populate automatically as pre-race feeds are released."
    : currentMeetingDisplayState === "FUTURE_MEETING_WITHOUT_FIELDS"
      ? "Fields are not available yet. EDGEiQ is monitoring this meeting and will populate intelligence automatically once runners are released."
      : marketStateContext;
  ...
  const tickerMeetings = useMemo(
    () =>
      currentMeeting
        ? [{ ...currentMeeting, races: currentMeetingRaces }]
        : meetings.filter((meeting) => meeting.races.length > 0),
    [currentMeeting, currentMeetingRaces, meetings]
  );
  ...
  const showOuterTerminalChrome = false;
    ...
    <>
      ...
      <div className="mx-auto max-w-[1760px] px-3 pb-3 pt-14 sm:px-4 sm:pt-3">
          ...
          <div className="grid gap-2 border-b border-[#263a53] px-3 py-2 lg:grid-cols-[260px_minmax(0,1fr)] lg:items-end">
                ...
                >
                  {meetings.map((meeting) => (
                    <option key={meeting.meetingKey} value={meeting.meetingKey}>
                      {displayDate(meeting.raceDate)} | {meeting.track}
                    </option>
                  ))}
                </select>
                {currentMeeting ? (
                  <div className="mt-1 flex flex-wrap items-center gap-1">
                    <span className="edgeiq-pill neutral">DATE {displayDate(currentMeeting.raceDate)}</span>
                    <span className="edgeiq-pill neutral">TRACK {currentMeeting.track}</span>
                    <span className={`edgeiq-pill ${meetingStatusToneClass(currentMeeting.meetingStatus)}`}>{currentMeetingStatusLabel}</span>
                    <span className="edgeiq-pill neutral">{currentMeetingRaceCount} RACES</span>
                  </div>
                ) : null}
              </label>
              <div>
                ...
                <div className="flex gap-1 overflow-x-auto pb-1">
                  ...
                  {currentMeeting && !currentMeetingRaces.length ? (
                    <div className="flex h-9 items-center rounded-full border border-slate-700 bg-[#0b1220] px-3 text-[11px] font-bold tracking-wide text-slate-400">
                      Monitoring field release
                    </div>
                  ) : null}
                </div>
              </div>
            ...
            {currentRace ? (
              <div className="edgeiq-current-race-detail">
                ...
                {currentRaceReliability ? (
                  <>
                    <span
                      className={`condition-pill edgeiq-race-reliability-pill ${raceReliabilityToneClass(currentRaceReliability.race_reliability_band_v1)}`}
                      title={text(currentRaceReliability.race_reliability_plain_english_v1) || "Rates how reliable this race is for the model based on historical race setup."}
                    >
                      {text(currentRaceReliability.race_reliability_display_label_v1) || "Race Reliability: Unknown"}
                    </span>
                    <span
                      className="edgeiq-race-reliability-hint"
                      title={text(currentRaceReliability.race_reliability_plain_english_v1) || "Rates how reliable this race is for the model based on historical race setup."}
                    >
                      {text(currentRaceReliability.race_reliability_ui_hint_v1) || "Unknown reliability"}
                    </span>
                  </>
                ) : null}
              </div>
              ...
              <div className="edgeiq-current-race-detail">
                <span>{displayDate(currentMeeting.raceDate)}</span>
                <span>{currentMeeting.track}</span>
                <span>{currentMeetingStatusLabel}</span>
                <span>{currentMeetingRaceCount ? `${currentMeetingRaceCount} races` : "fields pending"}</span>
              </div>
            ) : null}
          </div>
        ...
        {showOuterTerminalChrome && currentMeeting ? (
          <div className={`edgeiq-terminal-banner ${currentMeetingIsFuture ? "warn" : "good"}`}>
            <strong>{currentMeetingSummaryText}</strong>
            <span>{meetingNavigatorSummary}</span>
          </div>
        ) : null}
      ...
      <main className={showOuterTerminalChrome ? "mt-3" : "mt-0"}>
          ...
          {loading ? (
            ...
            <OverviewTab
              ...
              currentMeetingRaces={currentMeetingRaces}
            ...
            <EdgeiqRuntimeBoundary>
              <RaceIntelligenceScreen
                selectedTrack={currentMeeting?.track}
                selectedRaceNo={currentRace?.raceNo ?? ""}
                currentRace={currentRace}
                currentMeeting={currentMeeting}
                ...
                onProductViewChange={(next: "HOME" | "MEETINGS" | "RCE") => setIntelligenceProductView(next as "HOME" | "MEETING" | "RACE")}
              />
            </EdgeiqRuntimeBoundary>
          ) : tab === "MARKET" ? (
            <MarketTab
            ...
            <TrackingTab currentMeeting={currentMeeting} />
          ) : tab === "RESULTS" ? (
            <ResultsTab />
          ) : (
