  ...
  useEffect(() => {
    ...
    async function run(): Promise<void> {
      try {
        setLoading(true);
        setError("");
        const [raceCard, raceFields, executionEngine, rated, v8Candidate, liveBetQuality, finalRatings, history, summary, speed, silks, careerStats, fullCareer, scratchings, results, standards, raceRatingTargets, bias, raceReliability, trainerJockeyFactors, pacePressure, raceShape, sectionalTempo, activeSelector] = await Promise.all([
          ...
          loadCsvOptional("/data/edgeiq_live_runner_board_v1.csv"),
...
const silkByHorse = useMemo(() => {
  ...
  const currentRace = useMemo(() => {
    return (
      currentMeeting?.races.find((race) => race.key === selectedRaceKey) ??
      currentMeeting?.races[0] ??
      null
    );
  }, [currentMeeting, selectedRaceKey]);
