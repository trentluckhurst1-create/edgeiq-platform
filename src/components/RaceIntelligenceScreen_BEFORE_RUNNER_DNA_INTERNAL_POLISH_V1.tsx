...
export default function RaceIntelligenceScreen(props: Props): React.ReactElement {
  ...
  const selectedDnaScore = selected ? firstText(selected.dna, ["runner_dna_v6_1_score"], "—") : "—";
  const selectedDnaBand = selected ? firstText(selected.dna, ["runner_dna_v6_1_band"], "—").replace(/_/g, " ").toUpperCase() : "—";
  const selectedDnaRank = selected ? firstText(selected.dna, ["runner_dna_v6_1_rank_in_race"], "—") : "—";
  const selectedStrongestFactor = selected ? firstText(selected.dna, ["strongest_factor_v6_1"], "—").replace(/_/g, " ").toUpperCase() : "—";
  const selectedStrongestFactorScore = selected ? firstText(selected.dna, ["strongest_factor_score_v6_1"], "—") : "—";
  const selectedWeakestFactor = selected ? firstText(selected.dna, ["weakest_factor_v6_1"], "—").replace(/_/g, " ").toUpperCase() : "—";
  const selectedWeakestFactorScore = selected ? firstText(selected.dna, ["weakest_factor_score_v6_1"], "—") : "—";
  ...
  const selectedDnaNarrative = selected ? firstText(selected.dna, ["runner_dna_v6_1_narrative"], "") : "";
          ...
          <div style={breakdownGridStyle}>
            ...
            <div style={breakdownCardStyle}>
              ...
              <div style={metricRowStyle}>
                <span>DNA Score</span>
                <strong style={selectedIsScratched ? { color: "#94a3b8" } : { color: cellTone(selectedDnaBand) }}>
                  {selectedIsScratched ? "—" : selectedDnaScore}
                </strong>
              </div>
              <div style={metricRowStyle}>
                <span>Race Rank</span>
                <strong>{selectedIsScratched ? "—" : `#${selectedDnaRank}`}</strong>
              </div>
              <div style={metricRowStyle}>
                <span>Strongest</span>
                <strong style={{ color: "#7dd3fc" }}>
                  {selectedIsScratched ? "—" : `${selectedStrongestFactor} ${selectedStrongestFactorScore}`}
                </strong>
              </div>
              <div style={metricRowStyle}>
                <span>Weakest</span>
                <strong style={{ color: "#f5c451" }}>
                  {selectedIsScratched ? "—" : `${selectedWeakestFactor} ${selectedWeakestFactorScore}`}
                </strong>
              </div>
              <div style={metricRowStyle}>
                <span>Distance DNA</span>
                <strong style={selectedIsScratched ? { color: "#94a3b8" } : { color: cellTone(selectedDistanceBand) }}>
                  {selectedIsScratched ? "—" : `${selectedDistanceBand} ${selectedDistanceScore}`}
                </strong>
              </div>
              <div style={metricRowStyle}>
                <span>Condition DNA</span>
                <strong style={selectedIsScratched ? { color: "#94a3b8" } : { color: cellTone(selectedConditionBand) }}>
                  {selectedIsScratched ? "—" : `${selectedConditionBand} ${selectedConditionScore}`}
                </strong>
              </div>
              <div style={metricRowStyle}>
                <span>Class DNA</span>
                <strong style={selectedIsScratched ? { color: "#94a3b8" } : { color: cellTone(selectedClassBand) }}>
                  {selectedIsScratched ? "—" : `${selectedClassBand} ${selectedClassScore}`}
                </strong>
              </div>
              <div style={{ color: "#94a3b8", fontSize: 11, lineHeight: 1.4 }}>
                {selectedIsScratched ? "Runner scratched." : selectedDnaNarrative}
              </div>
            </div>
