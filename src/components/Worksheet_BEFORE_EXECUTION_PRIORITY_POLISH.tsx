...
function actionClass(v: unknown): string {
  ...
  if (s === "EXECUTE") return "exec-action execute";
  if (s === "LEAN") return "exec-action lean";
  if (s === "WATCH") return "exec-action watch";
  if (s === "SPEC WATCH") return "exec-action spec";
  if (s === "NO MARKET") return "exec-action no-market";
  return "exec-action pass";
}
        ...
        <div className="edgeiq-ws-head terminal-card-head">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" className="edgeiq-scr-toggle" onClick={() => setShowPass((v) => !v)}>
              {showPass ? "Hide PASS" : `Show PASS (${counts.pass})`}
            </button>
            <button type="button" className="edgeiq-scr-toggle" onClick={() => setShowNoMarket((v) => !v)}>
              {showNoMarket ? "Hide NO MARKET" : `Show NO MARKET (${counts.noMarket})`}
            </button>
          </div>
        </div>
        ...
        <div className="edgeiq-ws-table-wrap aligned-table-wrap worksheet-grid-wrap">
          <table className="edgeiq-ws-table aligned-runner-table worksheet-terminal-table worksheet-fixed-table">
            ...
            <tbody>
              ...
              {!rows.length && (
                <tr>
                  <td colSpan={14} className="empty">No active execution runners for this race under current filters.</td>
                </tr>
              )}
            </tbody>
          </table>
