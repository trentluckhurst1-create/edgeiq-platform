...
type MovementRow = {
  ...
  adaptive_execution?: string;
  adaptive_weight?: string | number;
    ...
    async function load() {
      try {
        ...
        const res = await fetch(
          `/adaptive_execution_board.csv?t=${Date.now()}`,
          { cache: "no-store" }
        );
  ...
  const stats = useMemo(() => {
    return {
      steamers: rows.filter((r) =>
        ["PRIORITY EXECUTE", "EXECUTE"].includes(safe(r.adaptive_execution).toUpperCase())
      ).length,

      drifters: rows.filter((r) =>
        safe(r.adaptive_execution).toUpperCase() === "AVOID"
      ).length,
    ...
    <section className="edgeiq-ws terminal-panel-stack">
      <div className="edgeiq-ws-card terminal-card">
        <div className="edgeiq-ws-head terminal-card-head">
          <div>
            ...
            <div className="text-[11px] text-[#7f8da0]">
              Adaptive execution · regime trust · market-state weighting
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2 mb-3">
          <Metric label="Execute" value={stats.steamers} />
          <Metric label="Tracked" value={stats.tracked} />
          <Metric label="Avoid" value={stats.drifters} />
              ...
              <tbody>
                {ranked.map((r, i) => (
                  <tr key={`${safe(r.track)}-${safe(r.race_no)}-${safe(r.horse)}-${i}`} className={`market-regime-row market-regime-row-${safe(r.market_regime).toLowerCase().replaceAll(" ", "-")}`}>
                    <td>
                      <span className={`adaptive-action-pill adaptive-action-${safe(r.adaptive_execution).toLowerCase().replaceAll(" ", "-")}`}>
                        {safe(r.adaptive_execution)}
                      </span>
                    </td>

                    <td className={`num strong ${edgeClass(r.adaptive_weight)}`}>
                      {safe(r.adaptive_weight)}
                    </td>
