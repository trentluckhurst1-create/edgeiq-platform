...
function edgeStrength(v: unknown): string {
  ...
  if (n >= 100) return "edge-hot";
  if (n >= 50) return "edge-good";
  if (n >= 0) return "edge-small";
  return "edge-bad";
}

function gapTone(v: unknown): string {
  const n = num(v);
  if (n === null) return "edgeiq-muted";
  if (n <= 2) return "edgeiq-gap-good";
  if (n <= 6) return "edgeiq-gap-watch";
  return "edgeiq-gap-bad";
}

function todayWidth(row: RatingDisplayRow): string {
  const today = num(row.todayRating);
  const peak = num(row.peak);
  if (today === null || peak === null || peak <= 0) return "0%";
  return `${Math.max(5, Math.min(100, (today / peak) * 100))}%`;
}
            ...
            <table className="edgeiq-ws-table edgeiq-ratings-table">
              ...
              <tbody>
                {ranked.map((row, idx) => {
                  ...
                  const edgeClass = edgeStrength(row.edgePct);
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="num strong">
                        <div className="edgeiq-form-bar">
                          <span className="peak" />
                          <span className="today" style={{ width: todayWidth(row) }} />
                        </div>
                      ...
                      <td className="num">
                        <span className={`edgeiq-edge-pill ${edgeClass}`}>
                          {row.isScratched ? "—" : pct(row.edgePct)}
                        </span>
                      </td>
                      <td className="num">{rating(row.peak)}</td>
                      <td className={`num strong ${gapTone(gap)}`}>{rating(gap)}</td>
          ...
          {selected ? (
            <>
              ...
              <div className="edgeiq-rating-price-grid">
                ...
                <div><span>Gap</span><strong className={gapTone(selectedGap)}>{rating(selectedGap)}</strong></div>
              </div>
