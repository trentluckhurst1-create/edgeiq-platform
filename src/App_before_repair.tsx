  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-4">
        ...
        {tab === "form" && (
          <FormTab
            runner={selectedHorseRunner}
            allRuns={fullCareerRunsForSelectedHorse}
            onSelectHorse={setSelectedHorseKey}
            raceRunners={currentRaceRunners}
          />
        )}
  ...
  if (!rows.length) {
    return (
      <div className="rounded-2xl border border-slate-800 bg-slate-950 p-4 text-sm text-slate-400">
        No rows available for this race.
      </div>
    );
  }
...
function FormTab({
  ...
  runner: WorksheetRunner | null;
  allRuns: FormRun[];
  raceRunners: WorksheetRunner[];
  ...
  const sortedRaceRunners = useMemo(() => {
    return [...raceRunners].sort(byHorseNo);
  }, [raceRunners]);

  const validRuns = useMemo(() => {
    return allRuns.filter(hasAnyFormData);
  }, [allRuns]);

  const totalPages = Math.max(1, Math.ceil(validRuns.length / pageSize));
  const pageRuns = validRuns.slice(page * pageSize, page * pageSize + pageSize);

  return (
    <section className="rounded-3xl border border-slate-800 bg-slate-900 p-4 md:p-5 shadow-xl">
      <div className="flex flex-col gap-4 xl:flex-row">
        <aside className="xl:w-72 rounded-2xl border border-slate-800 bg-slate-950 p-3">
          <div className="text-xs uppercase tracking-[0.22em] text-slate-500 mb-3">
            Horses
          </div>
          <div className="max-h-[420px] overflow-y-auto space-y-2 pr-1">
            {sortedRaceRunners.map((r) => {
              const active = r.horseKey === runner?.horseKey;
              return (
                <button
                  key={r.horseKey}
                  type="button"
                  ...
                  className={`w-full rounded-2xl border px-3 py-2 text-left transition ${
                    active
                      ? "border-white bg-[#07101d] text-slate-950"
                      : "border-slate-800 bg-slate-900 text-slate-200 hover:bg-slate-800"
                  }`}
                >
                  <div className="font-semibold">{r.horse}</div>
                  <div className="mt-1 text-xs opacity-80">
                    No {r.horseNo ?? ""} " Bar {r.barrier ?? ""}
                  </div>
                </button>
              );
            })}
        ...
        <div className="flex-1 space-y-4">
          <div className="rounded-2xl border border-slate-800 bg-slate-950 p-4">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
              <div>
                <div className="text-2xl font-bold">{runner?.horse ?? "Select a horse"}</div>
                {runner && (
                  <>
                    <div className="mt-2 text-sm text-slate-300">
                    ...
                    <div className="mt-2 text-sm text-slate-400">
                      Record view: full career available " 5 runs shown per window
                    </div>
                  </>
                )}
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={page === 0}
                  className="rounded-2xl border border-slate-700 bg-slate-900 px-3 py-2 text-sm disabled:opacity-40"
                >
                   Older
                </button>
                <div className="text-sm text-slate-400">
                  {validRuns.length ? `${page + 1} / ${totalPages}` : "0 / 0"}
                </div>
                <button
                  type="button"
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {pageRuns.map((run, idx) => (
                  <tr key={`${run.runDate}-${run.track}-${idx}`} className="border-t border-slate-800">
                    <td className="px-3 py-3 whitespace-nowrap">{run.runDate || ""}</td>
                    <td className="px-3 py-3 whitespace-nowrap">{run.runType || ""}</td>
                    <td className="px-3 py-3 whitespace-nowrap">{run.track || ""}</td>
                    <td className="px-3 py-3 whitespace-nowrap">
                      {run.distance !== null ? Math.round(run.distance) : ""}
                    </td>
                    <td className="px-3 py-3 whitespace-nowrap">{run.raceClass || ""}</td>
                    <td className="px-3 py-3 whitespace-nowrap">{run.trackCondition || ""}</td>
                    <td className="px-3 py-3 whitespace-nowrap">{run.finishPos || ""}</td>
                    <td className="px-3 py-3 whitespace-nowrap">{run.margin || ""}</td>
                    <td className="px-3 py-3 whitespace-nowrap">
                    ...
                    <td className="px-3 py-3 whitespace-nowrap">{run.sp || ""}</td>
                    <td className="px-3 py-3 whitespace-nowrap">
                      {run.runRating !== null ? run.runRating.toFixed(2) : ""}
                    </td>
                  </tr>
                ))}
                ...
                {!pageRuns.length && (
                  <tr>
                    <td className="px-3 py-6 text-slate-400" colSpan={13}>
                      No runs available.
                    </td>
                  </tr>
                )}
