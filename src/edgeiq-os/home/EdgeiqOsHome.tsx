...
function dayTabLabel(index: number, displayDate: string): { title: string; date: string } {
  const parsed = parseDisplayDate(displayDate);
  const fallbackTitle = index === 0 ? "Today" : index === 1 ? "Tomorrow" : "Sunday";
  if (!parsed) return { title: fallbackTitle, date: displayDate };
  ...
  return (
    <section className="eiq-dashboard-locked" aria-label="EDGEiQ Dashboard" data-edgeiq-workspace-key="HOME">
      <header className="eiq-dashboard-locked__header">
        <div>
          <h1>Dashboard</h1>
          <p>Race day command centre. Live meetings, key races, market context and alerts.</p>
        </div>
        <div className="eiq-dashboard-locked__days" aria-label="Racing days">
          {days.map((day, index) => {
            const label = dayTabLabel(index, day.displayDate);
            return (
              <button key={day.key} type="button" className={selectedDayIndex === index ? "is-active" : ""} onClick={() => setSelectedDayIndex(index)}>
                <strong>{label.title}</strong><span>{label.date}</span>
              </button>
            );
          })}
        </div>
      ...
      <div className="eiq-dashboard-locked__grid eiq-dashboard-locked__grid--top">
        <section className="eiq-dashboard-panel eiq-dashboard-panel--meetings">
          <header><strong>{selectedDayIndex === 0 ? "Today's" : dayTabLabel(selectedDayIndex, selectedDay?.displayDate ?? "").title} Meetings</strong><button type="button" onClick={onOpenMeetings}>View All</button></header>
          <div className="eiq-dashboard-table-wrap">
            <table className="eiq-dashboard-table">
              <thead><tr><th>Meeting</th><th>State</th><th>Rail</th><th>Track</th><th>Weather</th><th>Races</th><th>First</th><th>Status</th></tr></thead>
              <tbody>
                {meetings.map((meeting) => (
                  <tr key={meeting.meetingKey}>
                    <td><button className="eiq-dashboard-link" type="button" onClick={onOpenMeetings}>{display(canonicalTrackDisplayName(meeting.meeting))}</button></td>
                    <td>{display(meeting.state)}</td><td>{display(meeting.rail)}</td><td>{trackRating(meeting)}</td><td>{weatherSummary(meeting.weather)}</td><td>{meeting.races}</td>
                    <td>{display(meeting.raceSummaries?.[0]?.time)}</td><td><span className="eiq-dashboard-status">{meeting.races ? "READY" : "AWAITING"}</span></td>
                  </tr>
                ))}
        ...
        <section className="eiq-dashboard-panel">
          <header><strong>Race Activity</strong><span>Current programme</span></header>
          <div className="eiq-dashboard-activity">
            {activityRows.slice(0, 5).map((row, index) => <div key={`${row.meeting}-${row.race}-${index}`}><b>{row.race}</b><span>{row.meeting}</span><strong>{row.time}</strong></div>)}
            {!activityRows.length ? <p className="eiq-dashboard-empty">Awaiting race activity</p> : null}
          </div>
        </section>

        <section className="eiq-dashboard-panel">
          <header><strong>Alerts</strong><span>Governed feed</span></header>
          <div className="eiq-dashboard-alerts">
            {(selectedDay?.totals.scratchings ?? 0) > 0 ? <div className="is-alert"><i /> <span>{selectedDay?.totals.scratchings} scratchings in selected window</span></div> : <div><i /> <span>No scratching alerts in selected window</span></div>}
            <div><i /> <span>Track and weather status monitored</span></div>
            <div><i /> <span>Meeting feed {meetings.length ? "available" : "awaiting"}</span></div>
          </div>
        </section>
      </div>

      <div className="eiq-dashboard-locked__grid eiq-dashboard-locked__grid--middle">
        <section className="eiq-dashboard-panel eiq-dashboard-panel--featured">
          <header><strong>Featured Race</strong><span>{featuredMeeting ? canonicalTrackDisplayName(featuredMeeting.meeting) : "Awaiting race"}</span></header>
          {featuredMeeting && featuredRace ? <>
            <div className="eiq-dashboard-featured-head"><div><h2>{canonicalTrackDisplayName(featuredMeeting.meeting)} {featuredRace.label}</h2><p>{[featuredRace.raceClass, featuredRace.distance, trackRating(featuredMeeting), display(featuredMeeting.rail)].filter(Boolean).join("  ·  ")}</p></div><button type="button" onClick={() => onOpenWorkspace?.("race")}>Open Race Analysis →</button></div>
            <div className="eiq-dashboard-featured-tabs"><span className="is-active">Top Contenders</span><span>Market</span><span>Speed Map</span><span>Key Factors</span></div>
            <div className="eiq-dashboard-featured-empty">Open Race Analysis for governed runner ratings, prices and tactical intelligence.</div>
          </> : <div className="eiq-dashboard-empty">Awaiting featured race</div>}
        </section>

        <section className="eiq-dashboard-panel">
          <header><strong>Track & Weather</strong><span>Selected day</span></header>
          <div className="eiq-dashboard-mini-table">
            {meetings.slice(0, 5).map((meeting) => <div key={meeting.meetingKey}><strong>{canonicalTrackDisplayName(meeting.meeting)}</strong><span>{display(meeting.rail)}</span><span>{trackRating(meeting)}</span><span>{weatherSummary(meeting.weather)}</span></div>)}
          </div>
        </section>
      </div>
      ...
      <div className="eiq-dashboard-locked__grid eiq-dashboard-locked__grid--bottom">
        ...
        <section className="eiq-dashboard-panel"><header><strong>{selectedDayIndex === 0 ? "Today's" : "Race"} Timeline</strong><span>{selectedDay?.displayDate ?? "Current window"}</span></header><div className="eiq-dashboard-timeline">{activityRows.slice(0, 6).map((row, index) => <div key={`${row.meeting}-${row.race}-timeline-${index}`}><i /><time>{row.time}</time><span>{row.meeting} {row.race}</span><em>{row.status}</em></div>)}</div></section>
      </div>
    </section>
  );
}
