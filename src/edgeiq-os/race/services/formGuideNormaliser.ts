...
export type FormGuideRecentRun = {
  ...
  positionInRunningStatus: string;
  positionInRunningBySegment: Record<string, string>;
  movementBySegment: Record<string, string>;
  raceClass: string;
  margin: string;
  weight: string;
  jockey: string;
...
export type FormGuideRunnerDisplay = {
  ...
  earlySpeed: string;
  earlySpeedStatus: string;
  earlySpeedLabel: string;
  marketPrice: string;
  edgeiqPrice: string;
  edgeiqPriceStatus: string;
...
const ENGINE_LABELS: Record<string, string> = {
  ...
  ZERO_HISTORICAL_RUNS: "No History",
  HISTORICAL_RUNS_BUT_NO_SPEED_SOURCE: "No Speed Source",
  SPEED_SOURCE_PRESENT_BUT_NOT_METHODOLOGY_ELIGIBLE: "Not Eligible",
  HISTORY_FOUND_NO_RATEABLE_PERFORMANCE: "No Rateable EPR",
  NO_CURRENT_EPR_NO_PRICE: "No Current EPR",
...
function recordMap(value: unknown): Record<string, string> {
  ...
  return Object.fromEntries(
    Object.entries(parsed)
      .map(([key, entry]) => [safeText(key), safeText(entry)])
...
function compactUnavailableLabel(value: unknown, fallback = "Unavailable"): string {
  if (!isSourceValue(value)) return "";
  const raw = safeText(value.missingReason) || safeText(value.status);
  if (!raw || /^current$|^available$|^populated$/i.test(raw)) return "";
  const key = raw.toUpperCase();
  const labels: Record<string, string> = {
    NO_HISTORY: "No History",
    NO_GOVERNED_HISTORY: "No History",
    NO_CURRENT_EPR_NO_PRICE: "No Current EPR",
    NO_GOVERNED_PROJECTION: "No Projection",
    FIRST_STARTER: "First Starter",
    NO_HISTORICAL_EARLY_SPEED: "No Early Evidence",
    NO_ASOF_EVIDENCE: "No As-Of Evidence",
    INSUFFICIENT_RUNS: "Insufficient Runs",
    INSUFFICIENT_FACTOR_FAMILIES: "Insufficient Evidence",
    INSUFFICIENT_HISTORICAL_RUNS: "Insufficient History",
    OTHER: "Unavailable",
  };
  if (labels[key]) return labels[key];
  return humaniseEngineLabel(raw) || fallback;
}

function metricDisplayOrStatus(value: unknown, decimals: number | undefined, fallback = "Unavailable"): string {
  return metricText(value, decimals) || compactUnavailableLabel(value, fallback);
}
...
function recentRunsFromEnriched(enriched: EnrichedFormGuideRunner | undefined): FormGuideRecentRun[] {
  ...
  return enriched.fullForm.map((run) => {
    const runRecord = run as EnrichedFormRun & Record<string, unknown>;
    const positionMap = recordMap(firstValue(runRecord, ["positionInRunningBySegment", "position_in_running_by_segment", "inRunPositions", "in_run_positions"]));
    const movementMap = recordMap(firstValue(runRecord, ["movementBySegment", "movement_by_segment", "inRunMovement", "in_run_movement"]));
    return {
    ...
    positionInRunningStatus: safeText(firstValue(runRecord, ["positionInRunningStatus", "position_in_running_status"])),
    positionInRunningBySegment: positionMap,
    movementBySegment: movementMap,
    ...
    esiS8: signedMetricText(run.sectionalIndices?.indexStartTo800, 1),
  ...
  const runners = field
    .map((runner, index) => {
      ...
      const display: FormGuideRunnerDisplay = {
        id: safeText(enriched?.runnerId) || `race-${raceNumber || "race"}-runner-${no || normaliseRunnerName(name) || "unknown"}-${index + 1}`,
        sourceIndex: index,
        no,
        ...
        epi: metricDisplayOrStatus(enriched?.epr ?? enriched?.epi, 1, "No EPR"),
        epiStatus: compactUnavailableLabel(enriched?.epr ?? enriched?.epi, "No EPR"),
        epiRank: "",
        epiFieldAverage: "",
        epiDifference: "",
        earlySpeed: metricDisplayOrStatus(enriched?.earlySpeed, 0, "No Speed"),
        earlySpeedStatus: compactUnavailableLabel(enriched?.earlySpeed, "No Speed"),
        earlySpeedLabel: "Early Speed",
        marketPrice,
        edgeiqPrice: scratched ? "" : formatPrice(enriched?.edgeiqPrice) || compactUnavailableLabel(enriched?.edgeiqPrice, "No Price"),
        edgeiqPriceStatus: compactUnavailableLabel(enriched?.edgeiqPrice, "No Price"),
        suitabilityScore: metricDisplayOrStatus(enriched?.suitability, 0, "No Suitability"),
        suitabilityStatus: compactUnavailableLabel(enriched?.suitability, "No Suitability"),
        suitabilityLabel: "",
        shapeFit: humaniseEngineLabel(enriched?.raceShape) || metricText(enriched?.raceShape),
        late: metricDisplayOrStatus(enriched?.lateSpeed, 1, "No Late Speed"),
        lateStatus: compactUnavailableLabel(enriched?.lateSpeed, "No Late Speed"),
        formMomentum: metricDisplayOrStatus(enriched?.formMomentum, 1, "No Momentum"),
        formMomentumStatus: compactUnavailableLabel(enriched?.formMomentum, "No Momentum"),
