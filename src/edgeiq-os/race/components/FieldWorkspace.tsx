...
export function FieldWorkspace({ meeting, selectedRaceKey, onRaceChange }: FieldWorkspaceProps) {
  ...
  async function toggleRunner(runner: ThreeDayRunner, index: number) {
    ...
    if (!path) {
      setLoaded((current) => ({
        ...current,
        [key]: { runner, loading: false, error: "Recent-run detail is not supplied for this runner." },
      }));
      return;
    }
    ...
    setLoaded((current) => ({
      ...current,
      [key]: { runner, loading: true, error: "" },
    }));
    try {
      const fullRunner = await loadRunnerDetail(path);
      setLoaded((current) => ({
        ...current,
        [key]: { runner: fullRunner, loading: false, error: "" },
      }));
    } catch (error) {
      setLoaded((current) => ({
        ...current,
        [key]: { runner, loading: false, error: error instanceof Error ? error.message : "Runner detail unavailable" },
      }));
    }
  }
    ...
    <section className="eiq-field-v1" aria-label="Field workspace" data-edgeiq-workspace-key="FIELD">
      <header className="eiq-field-v1__header">
        <div>
          ...
          <h1>Field</h1>
          <span>Declared runners for {raceTitle(selectedRace)}.</span>
        </div>
      ...
      <section className="eiq-field-v1__card">
        ...
        <div className="eiq-field-v1__table-wrap">
          <table>
            <thead><tr><th>No.</th><th>Runner</th><th>Barrier</th><th>Jockey</th><th>Trainer</th><th>Weight</th><th /></tr></thead>
            <tbody>
              {selectedRace.runners.map((runner, index) => {
                const key = `${selectedRace.raceKey}:${officialNumber(runner)}:${index}`;
                const isExpanded = expandedKey === key;
                const state = loaded[key];
                const detailRunner = state?.runner ?? runner;
                const runs = recentRuns(detailRunner);
                return [
                  <tr key={`${key}:main`} className={isExpanded ? "is-expanded" : ""}>
                    <td><span className="eiq-field-v1__number">{officialNumber(runner)}</span></td>
                  ...
                  isExpanded ? (
                    <tr key={`${key}:detail`} className="eiq-field-v1__detail-row">
                      <td colSpan={7}>
                        {state?.loading ? <p className="eiq-field-v1__message">Loading recent runs…</p> : null}
                        {!state?.loading && state?.error ? <p className="eiq-field-v1__message">{state.error}</p> : null}
                        {!state?.loading && !state?.error && runs.length ? (
                          <div className="eiq-field-v1__runs">
                            <div className="eiq-field-v1__runs-head"><span>Date</span><span>Track</span><span>Dist.</span><span>Class</span><span>Finish</span><span>Rating</span></div>
                            {runs.map((run, runIndex) => {
                              const record = asRecord(run);
                              return <div className="eiq-field-v1__run" key={runIndex}>
