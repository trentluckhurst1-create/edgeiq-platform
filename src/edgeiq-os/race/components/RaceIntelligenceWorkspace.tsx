...
function eprStatusLabel(value: unknown): string {
...
export function RaceIntelligenceWorkspace({ raceBook, field, formGuide, raceKey, meetingRaces = [], clean, onBackToMeeting, onOpenRace, onOpenRunner }: Props) {
  ...
  const metadata = [["MEETING", meeting || "Meeting Not Published"], ["DATE", date], ["TIME", time], ["DISTANCE", distance || "Distance Not Published"], ["CLASS", raceClass || "Class Not Published"], ["TRACK", track], ["RAIL", rail], ["WEATHER", weather], ["PRIZEMONEY", prize]];

  return (
    <section className="eiq-race-v1" aria-label="Race command" data-edgeiq-race-workspace-v1="command-v2">
      <div className="eiq-race-v1__breadcrumb" aria-label="Race breadcrumb">
        <button type="button" onClick={onBackToMeeting}>MEETINGS</button><span aria-hidden="true">&gt;</span><strong>{meeting || "MEETING"}</strong><span aria-hidden="true">&gt;</span><strong>RACE {raceNo || "-"}</strong><span aria-hidden="true">&gt;</span><strong>RACE COMMAND</strong>
      </div>
      ...
      <section className="eiq-race-v1__matters" aria-label="Decision snapshot"><header><h2>DECISION SNAPSHOT</h2><span>Pre-race command view</span></header><div>{model.marketSnapshot.map((item) => <p key={item.label}><span aria-hidden="true" /><strong>{item.label}:</strong> {item.value}</p>)}</div></section>
      ...
      <section className="eiq-race-v1__midrow">
        <article className={`eiq-race-v1__speed ${hasSpeedMapEvidence ? "is-populated" : "is-empty-state"}`} aria-label="Speed map preview"><header><h2>SPEED MAP PREVIEW</h2><span>{hasSpeedMapEvidence ? "Governed current early speed" : "Current early-speed evidence not supplied"}</span></header>{hasSpeedMapEvidence ? <div className="eiq-race-v1__speed-grid">{model.speedMap.map((zone) => <div key={zone.zone} className="eiq-race-v1__speed-zone"><strong>{zone.zone}</strong><div>{zone.runners.slice(0, 5).map((runner) => <span key={`${zone.zone}-${runner.no}-${runner.runner}`}><b>{runner.no}</b>{runner.runner}{runner.earlySpeed ? <em>{runner.earlySpeed}</em> : null}</span>)}</div></div>)}</div> : <div className="eiq-race-v1__empty">Current early-speed evidence not supplied</div>}</article>
        <article className="eiq-race-v1__epi" aria-label="EPR top 3"><header><h2>EPR TOP 3</h2><span>{model.topEpr.length ? "Governed EPR" : "Insufficient Evidence"}</span></header><div>{[0, 1, 2].map((index) => {
          const row = model.topEpr[index];
          const statusLabel = row ? eprStatusLabel(row.status) : "";
          return row ? <div key={`${row.no}-${row.runner}`} className={`eiq-race-v1__epi-row ${statusLabel ? "has-status" : "is-normal"}`}><b>{index + 1}</b><span>{row.no}</span><strong>{row.runner}</strong><em>{row.value}</em><small>{statusLabel}</small></div> : <div key={`empty-epr-${index}`} className="eiq-race-v1__epi-row is-empty"><b>{index + 1}</b><span>-</span><strong>Insufficient Evidence</strong><em>-</em><small>Awaiting EPR</small></div>;
        })}</div></article>
      </section>

      <section className="eiq-race-v1__board" aria-label="Runner board"><header><h2>RUNNER BOARD</h2><span>{model.runnerBoard.length ? `${model.runnerBoard.length} runners · select a runner for deep form` : "Field Not Published"}</span></header><div className="eiq-race-v1__table-wrap"><table className="eiq-race-v1__table"><colgroup><col className="col-no" /><col className="col-silk" /><col className="col-runner" /><col className="col-bar" /><col className="col-wgt" /><col className="col-jockey" /><col className="col-trainer" /><col className="col-epi" /><col className="col-speed" /><col className="col-edgeiq" /><col className="col-market" /><col className="col-status" /></colgroup><thead><tr><th>NO</th><th>SILK</th><th className="is-left">RUNNER</th><th>BAR</th><th>WGT</th><th className="is-left">JOCKEY</th><th className="is-left">TRAINER</th><th>EPR</th><th>SPD</th><th>EDGEiQ PRICE</th><th>MARKET</th><th>STATUS</th></tr></thead><tbody>{model.runnerBoard.length ? model.runnerBoard.map((runner, index) => <tr key={`${runner.no}-${runner.runner}`} className={runner.scratched ? "is-scratched" : ""} role={runner.scratched ? undefined : "button"} tabIndex={runner.scratched ? undefined : 0} aria-label={runner.scratched ? undefined : `Open ${runner.runner} runner profile`} onClick={() => { if (!runner.scratched) onOpenRunner?.(index); }} onKeyDown={(event) => { if (!runner.scratched && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); onOpenRunner?.(index); } }}><td>{cell(runner.no)}</td><td><Silk src={runner.silkUrl} runner={runner.runner} /></td><td className="is-left"><strong>{cell(runner.runner)}</strong></td><td>{cell(runner.barrier)}</td><td>{cell(runner.weight)}</td><td className="is-left">{cell(runner.jockey)}</td><td className="is-left">{cell(runner.trainer)}</td><td>{runner.scratched ? "-" : cell(runner.epr)}</td><td>{runner.scratched ? "-" : cell(runner.earlySpeed)}</td><td>{runner.scratched ? "-" : cell(runner.edgeiqPrice)}</td><td>{runner.scratched ? "Scratched" : cell(runner.market)}</td><td><span className="eiq-race-v1__status">{runner.status}</span></td></tr>) : <tr><td colSpan={12}>Field Not Published</td></tr>}</tbody></table></div></section>
    </section>
  );
}
