...
type TabKey = "Execution" | "Market Tape" | "Ratings" | "Form" | "Speed Map" | "Intelligence" | "Performance" | "Bets";
type CsvRow = Record<string, string>;

const TAB_KEYS: TabKey[] = ["Execution", "Market Tape", "Ratings", "Form", "Speed Map", "Intelligence", "Performance", "Bets"];
...
function storedTab(): TabKey {
  ...
  return saved && TAB_KEYS.includes(saved) ? saved : "Execution";
}
  ...
  useEffect(() => {
    ...
    async function run(): Promise<void> {
      try {
        setLoading(true);
        setError("");
        const [raceCard, raceFields, rated, finalRatings, history, summary, speed, silks, careerStats, fullCareer, scratchings, results, standards, bias, pacePressure] = await Promise.all([    loadCsv("/data/race_card_report.csv"),
          loadCsvOptional("/data/race_fields.csv"),
          loadCsv("/data/ratings_final_v2.csv"),
...
const silkByHorse = useMemo(() => {
  ...
  const ratingRows = useMemo<RatingDisplayRow[]>(() => {
    const rows = raceCardRows
      ...
      .map((row, index) => {
        ...
        const marketStatus = marketPrice ? "OK" : (text(rated?.market_status) || text(row.market_status) || "MISSING").toUpperCase();
        ...
        return {
          ...
          track,
          raceNo,
          raceTime: text(mergedRow.race_time),
          horse,
          horseKey,
          matchKey,
          ...
          ratedPrice: firstPrice(num(finalRating?.elite_rated_price), ratedPrice),
          marketPrice,
          marketStatus,
          marketNote,
          highEdgeReview,
  ...
  }, [raceCardRows, ratedByRunner, finalRatingsByRunner, historyByHorse, summaryByHorse, silkByHorse, scratchMap, fieldScratchMap]);
  ...
  useEffect(() => {
    if (initialSelectionDone.current || !meetings.length) return;
    initialSelectionDone.current = true;
    setSelectedMeetingKey((current) => current || meetings[0].meetingKey);
    setSelectedRaceKey((current) => current || meetings[0].races[0]?.key || "");
  }, [meetings]);
  ...
  const currentSpeedRows = useMemo(() => {
    ...
    return speedRows
      ...
      .map((row) => {
        ...
        const fieldRunner = currentRace.rows.find((runner) => compactKey(runner.horseKey) === key || compactKey(runner.horse) === compactKey(row.horse));
        return { ...row, is_scratched: fieldRunner?.isScratched ? "1" : text(row.is_scratched), silkUrl: silkByHorse.get(key) ?? defaultSilkUrl() };
      });
...
return rows;
  ...
  return (
    <>      <EdgeRaceTicker meetings={meetings} currentRaceKey={currentRace?.key ?? ""} />      <div className="ert-shell min-h-screen bg-[#07101b] text-[#dbe7f3]">
        ...
        <header className="rounded-2xl border border-slate-700/60 bg-gradient-to-r from-[#0b1220] to-[#070d18] text-white shadow-xl">
          <div className="grid gap-2 border-b border-[#263a53] px-3 py-2 lg:grid-cols-[260px_minmax(0,1fr)] lg:items-end">
            <div>
              ...
              <div className="mt-0.5 text-[18px] font-semibold tracking-tight">Execution terminal</div>
            </div>
      ...
      <main className="mt-3">
          ...
          {loading ? (
            ...
            <RatingsTab runners={currentRace?.rows ?? []} selectedHorseKey={selectedRunner?.horseKey ?? ""} onSelectHorse={setSelectedHorseKey} historyByHorse={historyByHorse} careerStats={selectedCareerStats} raceStandard={currentRaceStandard} />
          ) : tab === "Form" ? (
            <FormTab
