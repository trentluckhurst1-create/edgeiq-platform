...
def main() -> None:
    ...
    for row in component_rows:
        ...
        if (
            component_type
            in components_by_adjusted_id[
                adjusted_id or text(row["race_entry_id"])
            ]
        ):
            duplicate_component_natural_keys.append(
                f"{adjusted_id or text(row['race_entry_id'])}:"
                f"{component_type}"
            )
    ...
    current_runner_mode = (
        not adjusted_rows
        and bool(fact_rows)
        and len(current_aggregate_rows) == len(fact_rows)
        and len(current_component_rows) == len(component_rows)
    )
    ...
    check(
        ...
        ),
        {
            ...
            "current_runner_aggregate_rows": (
                len(current_aggregate_rows)
            ),
    ...
    for row in fact_rows:
        ...
        if current_runner_mode and not adjusted_id:
            component = components_by_adjusted_id.get(
                text(row["race_entry_id"]),
                {},
            ).get(CURRENT_COMPONENT_TYPE)

            if component is None:
                component_set_errors.append(aggregate_id)
                continue
            ...
            aggregate_value = text(
                row["aggregate_suitability_value"]
            )

            component_decimal = parse_decimal(component_value)
            aggregate_decimal = parse_decimal(aggregate_value)

            if (
                component_decimal is None
                or aggregate_decimal is None
            ):
                arithmetic_errors.append(aggregate_id)

            elif component_decimal != aggregate_decimal:
                arithmetic_errors.append(aggregate_id)

            if text(row["suitability_component_count"]) != "1":
                governance_errors.append(
                    f"{aggregate_id}:component_count"
                )

            if (
                text(row["suitability_aggregate_decision"])
            ...
            if not aggregate_id.startswith("RESA-CUR-"):
                identity_errors.append(aggregate_id)

            expected_component_evidence_set = sha256_payload(
                [
                    text(
                        component[
            ...
            if text(
                row["source_component_evidence_set_sha256"]
            ...
            if text(
                ...
                evidence_errors.append(aggregate_id)
            ...
            required_lineage = [
                ...
                text(row["source_component_evidence_set_sha256"]),
                text(
                    row[
                        "race_entry_suitability_aggregate_evidence_sha256"
                    ]
                ...
                text(row["source_component_builder_version"]),
                text(row["builder_version"]),
                text(row["contract_version"]),
                text(row["built_at_utc"]),
            ]

            if any(not value for value in required_lineage):
                governance_errors.append(
