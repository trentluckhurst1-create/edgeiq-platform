...
def main():
    ...
    enriched=build_enriched_index(); current_market=build_current_market_index(); rows=[]; matched_enriched=0; matched_current_market=0
    for m in catalog.get('meetings',[]) if isinstance(catalog,dict) else []:
        ...
        for race in m.get('races',[]) or []:
            ...
            for i,runner in enumerate(race.get('runners',[]) or []):
                ...
                horse=runner_name(runner); key=(d,normalise(track),rn,normalise(horse)); en=enriched.get(key); cm=current_market.get(key); matched_enriched+=1 if en else 0; matched_current_market+=1 if cm else 0
                epi=value_from_source(en.get('epi')) if en else ''; edgeiq=value_from_source(en.get('edgeiqPrice')) if en else ''
