...
OUT = DATA / "edgeiq_live_track_intelligence_v2.csv"
SUMMARY = DATA / "edgeiq_live_track_intelligence_v2_summary.csv"
...
def choose_profile(dna, track_key, dist_bucket, condition_group, season):
    subset = dna[
    ...
    attempts = [
        ("TRACK_DISTANCE_CONDITION_SEASON", condition_group, season),
        ("TRACK_DISTANCE_SEASON", "", season),
        ("TRACK_DISTANCE_CONDITION", condition_group, ""),
        ("TRACK_DISTANCE", "", ""),
    ]

    for level, cond, sea in attempts:
        q = subset[subset["profile_level"] == level].copy()

        if cond != "":
            q = q[q["condition_group"].fillna("").map(clean).str.upper() == cond]
        if sea != "":
            q = q[q["season"].fillna("").map(clean).str.upper() == sea]
...
def comment(r):
    ...
    level = clean(r.get("track_profile_level", ""))
    db = clean(r.get("distance_bucket", ""))
    season = clean(r.get("season", ""))
    cond = clean(r.get("condition_group", ""))

    base = (
        f"Track DNA profile used: {level}. "
        f"For {r.get('track','')} {db}"
        f"{' ' + season if season else ''}"
        f"{' ' + cond if cond and cond != 'UNKNOWN' else ''}, "
        f"historical winners most often profile as {r.get('historical_run_style','UNKNOWN')}, "
        f"from {r.get('historical_barrier_lane','UNKNOWN')} barrier lanes, "
        f"with {r.get('historical_movement_profile','UNKNOWN')} movement."
    )

    runner = ""
    if clean(r.get("runner_run_style", "")):
        runner = (
            f" {horse} has Runner DNA as {r.get('runner_run_style')} with "
            f"{r.get('runner_movement_profile','UNKNOWN')} movement. "
            f"Style alignment: {r.get('run_style_alignment')}. "
            f"Barrier alignment: {r.get('barrier_alignment')}. "
            f"Movement alignment: {r.get('movement_alignment')}."
        )
    else:
        runner = (
            f" {horse} has no reliable historical Runner DNA, so this is a Track DNA-led assessment."
        )

    return base + runner + f" Track fit band: {r.get('track_fit_band')}."

def main():
    dna = pd.read_csv(TRACK_DNA, dtype=str).fillna("")
    live = pd.read_csv(RUNNER_DNA, dtype=str).fillna("")

    dna["track_key_join"] = dna["track"].map(canon)
    live["track_key_join"] = live["track"].map(canon)

    if "distance_bucket" not in live.columns:
        live["distance_bucket"] = live["distance"].map(distance_bucket)
    else:
        live["distance_bucket"] = live.apply(
            lambda r: clean(r.get("distance_bucket", "")) or distance_bucket(r.get("distance", "")),
            axis=1
        )
    ...
    if "season" not in live.columns:
        ...
        live["season"] = live.apply(
            lambda r: clean(r.get("season", "")) or season_from_date(r.get("race_date", "")),
            axis=1
        )

    if "condition_group" not in live.columns:
        live["condition_group"] = "UNKNOWN"
    else:
        live["condition_group"] = live["condition_group"].map(lambda x: clean(x).upper() or "UNKNOWN")
    ...
    for _, r in live.iterrows():
        profile = choose_profile(
            ...
            clean(r.get("condition_group", "")).upper() or "UNKNOWN",
            clean(r.get("season", "")).upper()
        )
        ...
        if profile is None:
            out.update({
                "track_dna_match_status_v2": "NO_TRACK_DNA_MATCH",
                "track_profile_level": "",
            ...
            out.update({
                "track_dna_match_status_v2": "MATCHED",
                ...
                "track_intelligence_label_v2": profile.get("track_dna_label", ""),
            })
        ...
        out["track_intelligence_comment_v2"] = comment(out)
        rows.append(out)
    ...
    summary = pd.DataFrame([
        ["status", "COMPLETE"],
        ["rows", len(outdf)],
        ["matched_rows", int((outdf["track_dna_match_status_v2"] == "MATCHED").sum())],
    ...
    print("[LIVE_TRACK_INTELLIGENCE_V2] COMPLETE")
    print(f"rows={len(outdf)}")
    print(f"matched={(outdf['track_dna_match_status_v2'] == 'MATCHED').sum()}")
