...
service = root / "src" / "services" / "productShellMeetingService.ts"
service.write_text(r'''import { cleanTrack, text } from "../utils/edgeiqFormat";
...
start = text.index(" const productShellMeetings = useMemo(() => {")
end = text.index("\n const selectedShellMeeting =", start)

replacement = ''' const productShellMeetings = useMemo(() => buildProductShellMeetings({
  productShellRaces,
  productMeetingRows,
  meetingCalendarRows,
  liveTrackIntelligenceRows,
...
import_line = 'import { buildProductShellMeetings } from "../services/productShellMeetingService";'
...
print("[PRODUCT_SHELL_MEETING_SERVICE_EXTRACT] complete")
