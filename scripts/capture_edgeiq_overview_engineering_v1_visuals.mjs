...
async function capture(name, selector = ".eiq-overview-v1", fullPage = true) {
  const file = path.join(outDir, `edgeiq_overview_engineering_v1_${name}.png`);
...
try {
  ...
  await clickFirst("Overview tab", [
    page.getByRole("tab", { name: /^OVERVIEW$/i }),
    page.getByRole("button", { name: /^OVERVIEW$/i }),
  ]);

  await capture("01_complete_overview");
  await capture("02_evidence_table", ".eiq-overview-v1-table-scroll", false);
  ...
  await fs.writeFile(
    path.join(outDir, "edgeiq_overview_engineering_v1_browser_notes.txt"),
    [`URL: ${baseUrl}`, "", "Console / capture notes:", ...logs].join("\n"),
    "utf8",
  );

  console.log("EDGEIQ_OVERVIEW_ENGINEERING_V1_SCREENSHOTS_WRITTEN");
} finally {
  await browser.close();
}
