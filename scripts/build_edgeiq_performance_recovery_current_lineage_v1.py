...
def audit():
    ...
    status='PASS' if m['current_epi_available_rows']>0 and m['current_eri_available_rows']>0 and m['fair_price_available_rows']>0 and m['market_terminal_edge_rows']>0 else 'FAIL'; wjson(REC/'edgeiq_performance_recovery_population_audit_v1.json',{'generated_at':TS,'status':status,'metrics':m}); wcsv(REC/'edgeiq_performance_recovery_population_audit_v1.csv',[{'metric':k,'value':v} for k,v in m.items()]+[{'metric':'status','value':status}],['metric','value']); return {'status':status,**m}

def readiness(a,stages):
    status='READY_WITH_GOVERNED_COVERAGE_LIMITATIONS' if a.get('status')=='PASS' else 'NOT_READY'
...
def main():
    ...
    print('FINAL_STATUS',status); return 0 if a.get('status')=='PASS' else 2
