...
feeds = [
    ...
    ROOT / "public" / "data" / "edgeiq_on_track_weather_governed_v1_2.json",
    ROOT / "public" / "data" / "edgeiq_victorian_track_weather_v1.json",
]
summary_fields = [
    "CURRENT CONDITION",
    "TRACK CONDITION",
...
checks = {
    ...
    "hourly_contract_current": "hourlyRows: []" in service,
    "impact_rows_present": all(row in service for row in impact_rows),
    "terminal_feeds_declared": all(
        feed in service
        for feed in [
            "edgeiq_metropolitan_weather_v1.json",
            "edgeiq_race_weather_v1.json",
