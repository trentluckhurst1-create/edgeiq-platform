...
FIELDNAMES = [
    ...
    "runner_number", "horse", "runner", "horse_key", "horse_canon", "runner_id", "race_entry_id", "horse_code", "barrier", "jockey",
...
def day_bucket_for_date(catalog: dict[str, Any], meeting_date: str) -> str:
    labels = ["TODAY", "TOMORROW", "DAY+2"]
    dates = catalog.get("dates") if isinstance(catalog.get("dates"), list) else []
    for index, value in enumerate(dates):
        if clean(value)[:10] == meeting_date:
            return labels[index] if index < len(labels) else f"DAY+{index}"
    return ""
...
def build_rows() -> list[dict[str, Any]]:
    ...
    for meeting in catalog.get("meetings", []):
        ...
        bucket = day_bucket_for_date(catalog, meeting_date)
        ...
        for race in meeting.get("races", []):
            ...
            for runner in runners:
                ...
                # ------------------------------------------------------------
                # EDGEIQ IDENTITY RECOVERY
                #
                # The product catalogue is the authoritative identity-bearing
                # source for current runners. Some merged/supplemental runner
                # records can retain the horse/race population while losing
                # source.id and source.horseCode.
                #
                # Recover those values from another runner in the catalogue
                # having the same meeting date, race number and canonical
                # horse name. This changes identity transport only; it does
                # not alter production model or pricing mathematics.
                # ------------------------------------------------------------

                if not clean(source.get("id")) or not clean(source.get("horseCode")):
                    recovery_source = None

                    for recovery_meeting in catalog.get("meetings", []):
                        if not isinstance(recovery_meeting, dict):
                            continue

                        recovery_date = clean(
                            recovery_meeting.get("date")
                            or recovery_meeting.get("raceDate")
                            or recovery_meeting.get("race_date")
                            or recovery_meeting.get("meetingDate")
                        )

                        if recovery_date != meeting_date:
                            continue

                        for recovery_race in recovery_meeting.get("races", []):
                            if not isinstance(recovery_race, dict):
                                continue

                            recovery_rno = race_no(
                                recovery_race.get("raceNumber")
                                or recovery_race.get("race_number")
                                or recovery_race.get("raceNo")
                                or recovery_race.get("race_no")
                            )

                            if str(recovery_rno) != str(rno):
                                continue

                            for recovery_runner in recovery_race.get("runners", []):
                                if not isinstance(recovery_runner, dict):
                                    continue

                                recovery_official = (
                                    recovery_runner.get("official")
                                    if isinstance(recovery_runner.get("official"), dict)
                                    else {}
                                )

                                recovery_candidate_source = (
                                    recovery_runner.get("source")
                                    if isinstance(recovery_runner.get("source"), dict)
                                    else {}
                                )

                                recovery_name = clean(
                                    recovery_official.get("runner")
                                    or recovery_candidate_source.get("horseName")
                                    or recovery_candidate_source.get("runnerName")
                                )

                                if canon_runner(recovery_name) != horse_key:
                                    continue

                                if (
                                    clean(recovery_candidate_source.get("id"))
                                    or clean(recovery_candidate_source.get("horseCode"))
                                ):
                                    recovery_source = recovery_candidate_source
                                    break
                    ...
                    if recovery_source is not None:
                        if not clean(source.get("id")):
                            source["id"] = recovery_source.get("id")

                        if not clean(source.get("runnerId")):
                            source["runnerId"] = recovery_source.get("runnerId")

                        if not clean(source.get("horseCode")):
                            source["horseCode"] = recovery_source.get("horseCode")
                ...
                rows.append({
                    ...
                    "race_entry_id": clean(source.get("id") or source.get("runnerId")),
                    "horse_code": clean(source.get("horseCode") or horse_obj.get("id")),
                    "barrier": barrier,
                    "jockey": jockey,
                    "trainer": trainer,
                    "weight": weight,
