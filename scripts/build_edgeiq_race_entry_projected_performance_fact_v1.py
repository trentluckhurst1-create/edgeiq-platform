    ...
    for aggregate in aggregate_rows:
        ...
        if not adjusted_id:
            if not aggregate_id.startswith("RESA-CUR-"):
                fail(
                    f"{aggregate_id}: only governed current-runner "
                    "suitability aggregates may omit adjusted "
                    "performance."
                )
            ...
            historical_rating = parse_required_decimal(
                aggregate["historical_rating_value"],
                "historical_rating_value",
            )

            aggregate_suitability = parse_required_decimal(
                aggregate["aggregate_suitability_value"],
                "aggregate_suitability_value",
            )
            ...
            if (
                ...
                fail(
                    f"{aggregate_id}: incomplete current-runner "
                    "source lineage."
                )
            ...
            projected_id = row_id(
                "REPP-CUR",
                [
                    aggregate["race_entry_id"],
                    aggregate["race_id"],
                    aggregate["canonical_horse_id"],
                    aggregate[
                        "context_adjusted_performance_value"
                    ],
                ],
            ...
            context_adjusted_text = text(
                aggregate["context_adjusted_performance_value"]
            )
            projected_text = context_adjusted_text
            projected_delta_text = decimal_text(
                projected_performance - historical_rating
            )
            ...
            output_rows.append(
                {
                    ...
                    "race_id": text(aggregate["race_id"]),
                    "race_date": text(aggregate["race_date"]),
                    "runner_id": text(aggregate["runner_id"]),
                    "canonical_horse_id": text(
                        aggregate["canonical_horse_id"]
                    ),
                    "canonical_horse_name": text(
                        aggregate["canonical_horse_name"]
                    ),
                    "historical_rating_value": historical_text,
                    "context_parameter_id": text(
                        aggregate["context_parameter_id"]
                    ),
                    "total_context_adjustment": "",
                    "aggregate_suitability_value": aggregate_text,
                    "context_adjusted_performance_value": (
