...
OUT = DATA / "edgeiq_trainer_context_research_v1.csv"
SUMMARY = DATA / "edgeiq_trainer_context_research_v1_summary.csv"
...
for (trainer, ctype, cval), g in groups.items():
    starts = g["starts"]
    ...
    if signal == "CONTEXT_EDGE":
        phrase = f"{trainer} has historically performed above their own baseline for {ctype.lower()} {cval}."
    elif signal == "MILD_CONTEXT_EDGE":
        phrase = f"{trainer} has shown a mild positive context profile for {ctype.lower()} {cval}."
    elif signal == "CONTEXT_RISK":
        phrase = f"{trainer} has historically performed below their own baseline for {ctype.lower()} {cval}."
    else:
        phrase = f"{trainer} has a neutral context profile for {ctype.lower()} {cval}."
...
out_rows.sort(key=lambda r: (signal_order.get(r["signal"],9), -int(r["starts"]), r["trainer"]))
...
print("[EDGEIQ_TRAINER_CONTEXT_RESEARCH_V1] COMPLETE")
print(f"out={OUT}")
