...
ROOT=Path(__file__).resolve().parents[1]; DATA=ROOT/'public'/'data'; DOC=ROOT/'docs'/'victoria-live-recovery-v3'; RAW=DOC/'raw-evidence'; CANONICAL_HORSE_IDENTITY=ROOT/'docs'/'performance-intelligence'/'canonical-identities'/'edgeiq_canonical_horse_identity_v1.csv'
...
def load_horse_names():
    if not CANONICAL_HORSE_IDENTITY.exists(): raise RuntimeError(f'Missing governed canonical horse identity registry: {CANONICAL_HORSE_IDENTITY}')
    names={}
    for r in read_csv(CANONICAL_HORSE_IDENTITY):
        if clean(r.get('identity_domain'))!='horse' or clean(r.get('resolution_status'))!='RESOLVED': continue
        cid=clean(r.get('canonical_identity')); cname=clean(r.get('canonical_display_name'))
        if not cid: raise RuntimeError('Blank canonical horse ID in governed identity registry.')
        if not cname: raise RuntimeError(f'Blank canonical horse name in governed identity registry for {cid}.')
        if cid in names and names[cid]!=cname: raise RuntimeError(f'Conflicting canonical horse names in governed identity registry for {cid}: {names[cid]!r} != {cname!r}')
        names[cid]=cname
    return names
...
def rebuild_downstream(dry):
    timing=read_csv(DATA/'edgeiq_canonical_historical_timing_warehouse_v1.csv'); std=read_csv(DATA/'edgeiq_standard_time_fact_v1.csv'); par=read_csv(DATA/'edgeiq_length_conversion_parameter_fact_v2.csv'); horse_names=load_horse_names(); built=now()
    ...
    for t in timing:
        ...
        rk=f"{clean(t.get('race_date'))}|{clean(t.get('track')).upper()}|R{clean(t.get('race_number')).zfill(2)}"; rid='RTDR-'+kh([rk,t.get('canonical_race_id'),st.get('standard_time_id')]); rh=hashlib.sha256((clean(t.get('source_row_evidence_sha256'))+clean(st.get('standard_time_evidence_sha256'))+fmt(dd)).encode()).hexdigest(); whid=clean(t.get('winner_canonical_horse_id'))
        if not whid: raise RuntimeError(f"{rk}: blank winner_canonical_horse_id in canonical historical timing warehouse.")
        whname=clean(t.get('winner_horse_name')) or horse_names.get(whid)
        if not whname: raise RuntimeError(f"{rk}: winner_canonical_horse_id {whid!r} has no governed canonical display name.")
        d={'race_time_delta_id':rid,'benchmark_observation_id':clean(t.get('recovered_timing_observation_id')),'benchmark_group_id':clean(st.get('benchmark_group_id')),'standard_time_id':clean(st.get('standard_time_id')),'race_key':rk,'race_date':clean(t.get('race_date')),'track_name':clean(t.get('track')),'official_distance_metres':clean(t.get('distance_metres')),'winner_canonical_horse_id':whid,'winner_horse_name':whname,'winner_race_time_seconds':fmt(wt),'standard_time_seconds':fmt(ss),'time_delta_seconds':fmt(dd),'time_delta_interpretation':dint(dd),'calculation_method':'WINNER_RACE_TIME_MINUS_RECOVERED_STANDARD_TIME_SECONDS','source_observation_sha256':clean(t.get('source_row_evidence_sha256')),'source_standard_time_evidence_sha256':clean(st.get('standard_time_evidence_sha256')),'race_time_delta_evidence_sha256':rh,'builder_version':'edgeiq_race_time_delta_recovered_v1.1.0','contract_version':'1.0.0','built_at_utc':built}; deltas.append(d)
