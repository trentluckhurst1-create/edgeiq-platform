...
const MANIFEST = path.join(
  RECOVERY_DIR,
  "edgeiq_manifest_RECOVERY_20260625_20260819.json"
);
...
const SUMMARY = path.join(
  OUTPUT_DIR,
  "edgeiq_graphql_recovery_20260625_20260819_results_v1_summary.csv"
);

const AUDIT = path.join(
  OUTPUT_DIR,
  "edgeiq_graphql_recovery_20260625_20260819_results_v1_audit.csv"
);
...
(async () => {
  ...
  for (const meetItem of meetings) {
    ...
    console.log("[OPEN]", meetItem.meeting_url);

    try {
      await page.goto(meetItem.meeting_url, { waitUntil: "load", timeout: 90000 });
    ...
    const meetingCap = captures.find(x => x.operation === "getMeeting_CD");
    const racesCap = captures.find(x => x.operation === "getRacesForMeet_CD");
    ...
    if (!meetingCap || !racesCap) {
      audit.push({
        ...
        note: "missing getMeeting_CD or getRacesForMeet_CD",
        races: 0,
        rows: 0
      });
      continue;
    ...
    for (const race of races) {
      ...
      for (const e of entries) {
        ...
        allRows.push({
          ...
          source: "RACING_COM_GRAPHQL_RECOVERY_20260625_20260819",
