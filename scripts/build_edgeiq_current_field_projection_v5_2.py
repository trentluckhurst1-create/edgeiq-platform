...
TARGET_FORMULA = "class_par_rating_v5_2 + 0.15*distance_adjustment_v5_1 + 0.15*condition_adjustment_v5_1"
...
def clean_class(value: object) -> str:
    return canonicalize_race_class(value)[0]


def class_family(clean: str) -> str:
    return canonical_class_family(clean)
...
def main() -> None:
    ...
    for column in correction_keep_columns:
        if column not in correction.columns:
            correction[column] = ""

    correction_keep = correction[correction_keep_columns].drop_duplicates(
        ["race_date", "track", "race_no", "horse_match_key_v5_2"],
        keep="first",
    )
    ...
    current["canonical_class_for_par_v5_1"] = current["canonical_class_for_par_v5_1"].where(
        current["canonical_class_for_par_v5_1"].astype(str).str.strip().ne(""),
        current["corrected_race_class_v5_1"],
    )
    current["canonical_class_for_par_v5_1"] = current["canonical_class_for_par_v5_1"].map(clean_class)
    ...
    if "HANDICAP" not in set(class_keep["corrected_race_class_v5_1"].astype(str)):
        bm70 = class_keep[class_keep["corrected_race_class_v5_1"].astype(str).eq("BM70")].head(1).copy()
        if not bm70.empty:
            bm70.loc[:, "corrected_race_class_v5_1"] = "HANDICAP"
            bm70.loc[:, "class_par_confidence_v5_2"] = "MEDIUM"
            bm70.loc[:, "class_par_method_v5_2"] = "HANDICAP_FALLBACK_FROM_BM70"
            class_keep = pd.concat([class_keep, bm70], ignore_index=True)
    ...
    for _, row in current.iterrows():
        ...
        starts_found = int(metrics["starts_found_v5_2"])
        if pd.notna(projected):
            if starts_found == 1:
                projected = (0.35 * float(projected)) + 42.0
                method = method + " | LOW_SAMPLE_SHRINK_1_START"
            elif starts_found == 2:
                projected = (0.55 * float(projected)) + 30.0
                method = method + " | LOW_SAMPLE_SHRINK_2_START"
            elif starts_found == 3:
                projected = (0.75 * float(projected)) + 15.0
                method = method + " | LOW_SAMPLE_SHRINK_3_START"
        ...
        if target_missing_parts:
            ...
            target = float(class_par) + (0.15 * float(distance_par)) + (0.15 * float(condition_par))
            target_status = "TARGET_BUILT"
            target_method = "CLASS_PAR_PLUS_DISTANCE_CONDITION_ADJUSTMENTS"
    ...
    audit_rows: list[dict[str, object]] = [
        ...
        audit_row("overall", "class_par_join_matched_runners", int(out["class_par_rating_v5_2"].notna().sum())),
        audit_row("overall", "class_par_join_missing_runners", int(out["class_par_rating_v5_2"].isna().sum())),
