...
LVS = DATA / "edgeiq_results_lengths_v_standard_v1.csv"
ELAPSED = DATA / "edgeiq_results_elapsed_time_observations_v1.csv"
OUT = DATA / "edgeiq_runner_sectional_performance_v1.csv"
SUMMARY = DOCS / "edgeiq_runner_sectional_performance_v1_summary.json"
REPORT = DOCS / "edgeiq_runner_sectional_performance_v1_report.md"
...
FIELDS = [
    ...
    "early_lengths_vs_standard",
    "mid_lengths_vs_standard",
    "late_lengths_vs_standard",
    "final_segment_lengths_vs_standard",
    "best_segment_lengths_vs_standard",
    "worst_segment_lengths_vs_standard",
...
def segment_bucket(row: dict[str, str]) -> str:
    ...
    for (race_id, runner_id), rows in sorted(grouped.items()):
        ...
        for row in calculated:
            if clean(row.get("lengths_vs_standard")):
                by_bucket[segment_bucket(row)].append(Decimal(clean(row.get("lengths_vs_standard"))))
        first = rows[0]
        reason = ""
        if not calculated:
            reasons = sorted({clean(row.get("conversion_reason")) for row in rows if clean(row.get("conversion_reason"))})
            reason = ";".join(reasons)
        output.append({
            ...
            "matched_segment_count": len(rows),
            "eligible_segment_count": eligible_counts.get((race_id, runner_id), 0),
            "calculated_segment_count": len(calculated),
            "coverage_status": "CALCULATED" if calculated else "LENGTH_CONVERSION_SCOPE_BLOCKED",
            "coverage_reason": reason,
        })
    write_csv(OUT, output, FIELDS)
    summary = {
        "elapsed_incremental_observations_preserved": len(elapsed),
        "runner_sectional_rows": len(output),
        "calculated_runner_rows": sum(1 for row in output if row["coverage_status"] == "CALCULATED"),
        "blocked_runner_rows": sum(1 for row in output if row["coverage_status"] != "CALCULATED"),
        "status": "SECTIONAL_PERFORMANCE_SCOPE_BLOCKED_FOR_CURRENT_ROWS" if output and all(row["coverage_status"] != "CALCULATED" for row in output) else "SECTIONAL_PERFORMANCE_BUILT",
    }
    SUMMARY.write_text(json.dumps(summary, indent=2, ensure_ascii=True) + "\n", encoding="utf-8")
    REPORT.write_text(
        "# Runner Sectional Performance V1\n\n"
        f"Status: `{summary['status']}`\n\n"
        f"Elapsed incremental observations preserved: `{summary['elapsed_incremental_observations_preserved']}`\n\n"
        f"Runner rows: `{summary['runner_sectional_rows']}`\n\n"
        f"Calculated runner rows: `{summary['calculated_runner_rows']}`\n",
        encoding="utf-8",
    )
