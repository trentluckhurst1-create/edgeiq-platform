...
OUT = DATA / "edgeiq_current_field_projection_V6_1_RESEARCH_BACKFILL_FILTER_V3_20260617.csv"
SUMMARY = DATA / "edgeiq_current_field_projection_V6_1_RESEARCH_BACKFILL_FILTER_V3_20260617_summary.csv"
MIGRATION = DATA / "edgeiq_current_field_projection_V6_1_RESEARCH_BACKFILL_FILTER_V3_20260617_band_migration.csv"
...
def main():
    ...
    for _, r in curr.iterrows():
        ...
        if genuine_available > 0:
            ...
            projected, method = project(vals, " | BACKFILL_FILTER_V3_EXCLUDED_BACKFILLED")
            source_mode = "GENUINE_HISTORY_ONLY_BACKFILLED_EXCLUDED"
            action = "KEEP_FILTERED_V6_1_RESEARCH"
            starts_found = len(vals)
        ...
        elif backfilled_available > 0:
            projected = num(r.get("projected_rating_v5_2", ""))
            gap = num(r.get("projection_gap_v5_2", ""))
            source_mode = "BACKFILLED_ONLY_FALLBACK_TO_V5_2"
            action = "FALLBACK_TO_V5_2_BACKFILLED_ONLY"
            starts_found = backfilled_available

            out["projected_rating_V6_1_RESEARCH"] = round(projected, 2) if pd.notna(projected) else ""
            out["projection_gap_V6_1_RESEARCH"] = round(gap, 2) if pd.notna(gap) else ""
            out["projection_band_V6_1_RESEARCH"] = str(r.get("projection_band_v5_2", ""))
            out["projection_method_research"] = str(r.get("projection_method_v5_2", "")) + " | BACKFILL_FILTER_V3_BACKFILLED_ONLY_FALLBACK_TO_V5_2"

        else:
            source_mode = "NO_RESEARCH_HISTORY"
            action = "NO_RESEARCH_HISTORY"
            starts_found = 0
            out["projected_rating_V6_1_RESEARCH"] = ""
            out["projection_gap_V6_1_RESEARCH"] = ""
            out["projection_band_V6_1_RESEARCH"] = "NO_PROJECTION"
            out["projection_method_research"] = "NO_HISTORY | BACKFILL_FILTER_V3"

        out["starts_found_research"] = starts_found
        out["backfill_filter_v3_source_mode"] = source_mode
        out["backfill_filter_v3_action"] = action
        out["built_at_research_replay"] = built_at

        old_gap = num(r.get("projection_gap_v5_2", ""))
        new_gap = num(out.get("projection_gap_V6_1_RESEARCH", ""))
        out["projection_gap_delta_research_minus_v5_2"] = round(new_gap - old_gap, 2) if pd.notna(new_gap) and pd.notna(old_gap) else ""
    ...
    summary_rows = [
        {"section":"overall","metric":"status","value":"BACKFILL_FILTER_V3_BUILT"},
        {"section":"overall","metric":"rows","value":len(out_df)},
    ]

    for mode, count in out_df["backfill_filter_v3_source_mode"].value_counts(dropna=False).items():
        summary_rows.append({"section":"source_mode","metric":mode,"count":int(count)})

    for action, count in out_df["backfill_filter_v3_action"].value_counts(dropna=False).items():
        summary_rows.append({"section":"action","metric":action,"count":int(count)})
    ...
    print("[PROJECTION_V6_1_BACKFILL_FILTER_V3] COMPLETE")
