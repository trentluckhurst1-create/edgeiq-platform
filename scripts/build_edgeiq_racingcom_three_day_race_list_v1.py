...
FIELDS = [
    ...
    "meet_code", "track_code", "meet_status", "full_status", "state", "url_segment", "form_url",
...
def norm_track(value):
    s = clean(value).upper()
    for token in (
        "SPORTSBET-", "SPORTSBET ", "LADBROKES ", "BET365 ", "BET365-",
        "SOUTHSIDE ", "PICKLEBET ", "PICKLEBET-",
    ):
        s = s.replace(token, "")
    s = re.sub(r"^PARK\s+", "", s)
...
def day_bucket(value):
    try:
        race_date = datetime.fromisoformat(value[:10]).date()
    except Exception:
        return ""
    delta = (race_date - TARGET_DATES[0]).days
    return {0: "TODAY", 1: "TOMORROW", 2: "DAY+2"}.get(delta, "")
...
def extract_calendar_meetings():
    ...
    for year, month in month_targets():
        ...
        all_entries.append({"url": url, "payload": fetch_json(url)})
    ...
    for bundle in all_entries:
        for month in (bundle["payload"].get("Months") or []):
            for entry in (month.get("CalendarEntries") or []):
                ...
                if state != "VIC":
                    continue
                if entry.get("IsTrial") or entry.get("IsJumpout"):
                    continue

                race_date = clean(entry.get("Date"))[:10]
                bucket = day_bucket(race_date)
                if bucket not in {"TODAY", "TOMORROW", "DAY+2"}:
                    continue

                meet_code = clean(entry.get("MeetCode"))
                url_segment = clean(entry.get("UrlSegment"))
                # Racing.com can expose a live form page before HasRaceInformation flips true.
                # Keep a current VIC meeting whenever we have enough identity to query it.
                if not (meet_code or url_segment):
                    continue

                track_raw = clean(entry.get("Track") or entry.get("Venue"))
                ...
                wanted[key] = {
                    ...
                    "meeting_key": key,
                    "meet_code": meet_code,
                    "track_code": clean(entry.get("TrackCode")),
                    "meet_status": clean(entry.get("MeetStatus")),
                    "full_status": clean(entry.get("FullStatus")),
                    "state": state,
                    "url_segment": url_segment,
                    "form_url": "https://www.racing.com/form/" + url_segment if url_segment else "",
                }
...
query getRaceNumberList_CD($meetCode: ID!) {
  getNoCacheRacesForMeet(meetCode: $meetCode) {
    id raceNumber raceStatus distance time name nameForm trackCondition trackRating rdcClass
    isTrial isJumpOut trackCode formRaceEntries { horseName }
    meet { venue meetUrl meetUrlSegment }
  }
}
"""
    import urllib.parse
    return "https://graphql.rmdprod.racing.com/?" + urllib.parse.urlencode(
        {"query": query, "variables": json.dumps({"meetCode": str(meet_code)})}
    )
...
def fetch_races_browser(meetings):
    ...
    with sync_playwright() as playwright:
        ...
        context = browser.new_context(user_agent="Mozilla/5.0", viewport={"width": 1280, "height": 900})
        ...
        for meeting in meetings:
            ...
            def handle_response(response):
                if "graphql.rmdprod.racing.com" not in response.url:
                    return
                try:
                    data_obj = (json.loads(response.text()).get("data") or {})
            ...
            if meeting.get("form_url"):
                ...
                try:
                    page.goto(meeting["form_url"], wait_until="domcontentloaded", timeout=45000)
            ...
            if not races and meeting.get("meet_code"):
                page = context.new_page()
                try:
                    page.goto(gql_url_for_race_list(meeting["meet_code"]), wait_until="networkidle", timeout=45000)
                    body = page.locator("body").inner_text(timeout=10000)
                    races = ((json.loads(body).get("data") or {}).get("getNoCacheRacesForMeet") or [])
                except Exception:
                    races = []
                page.close()

            def capture_race_detail(race_no_value):
                if not meeting.get("form_url"):
                    return
                ...
                def handle_detail_response(response):
                    if "graphql.rmdprod.racing.com" not in response.url:
                        return
                    try:
                        detail = ((json.loads(response.text()).get("data") or {}).get("getRaceForm"))
                        if isinstance(detail, dict) and detail.get("formRaceEntries"):
                            for key in {clean(detail.get("id")), clean(detail.get("raceCode")), clean(detail.get("raceNumber"))}:
                                if key:
                                    race_form_payloads[key] = detail
                ...
                try:
                    detail_page.goto(f"{meeting['form_url']}/race/{race_no_value}", wait_until="domcontentloaded", timeout=45000)
            ...
            for race in races:
                ...
                if race_no_value and not (race_form_payloads.get(clean(race.get("id"))) or race_form_payloads.get(race_no_value)):
                    capture_race_detail(race_no_value)

            for race in races:
                if race.get("isTrial") or race.get("isJumpOut"):
                    continue
                race_no_value = clean(race.get("raceNumber"))
                if not race_no_value:
                    continue
                detailed = race_form_payloads.get(clean(race.get("id"))) or race_form_payloads.get(race_no_value)
                merged = merge_race(race, detailed)
                entries = merged.get("formRaceEntries") or []
                rows.append({
                    **meeting,
                    "race_id": clean(merged.get("id")),
                    "race_no": race_no_value,
                    "race_name": clean(merged.get("name")),
                    "race_class": clean(merged.get("rdcClass") or merged.get("class") or merged.get("nameForm")),
                    "distance": clean(merged.get("distance")),
                    "race_time_utc": clean(merged.get("time")),
                    "race_status": clean(merged.get("raceStatus") or merged.get("status")),
                    "track_condition": clean(merged.get("trackCondition") or merged.get("condition")),
                    "track_rating": clean(merged.get("trackRating")),
                    "rail_position": "",
                    "weather": "",
...
def main():
    ...
    rows.sort(
        key=lambda row: (
            {"TODAY": 0, "TOMORROW": 1, "DAY+2": 2}.get(row.get("day_bucket", ""), 9),
            row.get("race_date", ""),
            row.get("normalised_track", ""),
            int(row.get("race_no") or 999),
        )
    )
    ...
    summary = [
        ...
        {"metric": "meetings_with_races", "value": len(set(row["meeting_key"] for row in rows))},
        {"metric": "today_meetings_with_races", "value": len(set(row["meeting_key"] for row in rows if row.get("day_bucket") == "TODAY"))},
    ...
    print("meetings_with_races=", len(set(row["meeting_key"] for row in rows)))
    print("today_meetings_with_races=", len(set(row["meeting_key"] for row in rows if row.get("day_bucket") == "TODAY")))
    print("today_races=", sum(1 for row in rows if row.get("day_bucket") == "TODAY"))
    print("out=", OUT)
