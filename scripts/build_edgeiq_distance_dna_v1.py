...
OUT = DATA / "edgeiq_distance_dna_v1.csv"
SUMMARY = DATA / "edgeiq_distance_dna_v1_summary.csv"
...
def add(entity_type, entity_name, distance_bucket, distance_m, row):
    if not entity_name or distance_bucket == "UNKNOWN":
        return

    key = (entity_type, norm(entity_name), distance_bucket)
...
with SRC.open("r", encoding="utf-8-sig", newline="") as f:
    for row in csv.DictReader(f):
        ...
        add("HORSE", horse, db, distance_m, row)
        add("TRAINER", trainer, db, distance_m, row)
        add("JOCKEY", jockey, db, distance_m, row)

        if trainer and jockey:
            add("CONNECTION", f"{trainer} | {jockey}", db, distance_m, row)
...
summary = [
    {"metric": "status", "value": "EDGEIQ_DISTANCE_DNA_V1_BUILT"},
    {"metric": "source", "value": str(SRC)},
    {"metric": "source_rows_with_distance", "value": distance_rows},
...
print("[DISTANCE_DNA_V1_FIXED] COMPLETE")
print(f"source_rows_with_distance={distance_rows}")
