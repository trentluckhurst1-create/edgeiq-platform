...
DB = DATA / "edgeiq_sectional_warehouse_v3.sqlite"
WAREHOUSE = DATA / "edgeiq_sectional_warehouse_v3.csv"
QUARANTINE = DATA / "edgeiq_sectional_quarantine_v3.csv"
SUMMARY = DATA / "edgeiq_sectional_warehouse_v3_summary.csv"
RECOVERY_AUDIT = DATA / "edgeiq_sectional_identity_recovery_v3_files.csv"
PATTERNS = ("sectional", "racingcom", "racing_com", "split", "tempo")
SKIP_PARTS = {"node_modules", "dist", ".git", "__pycache__", "checkpoints"}
...
SKIP_FILES = {
    "edgeiq_sectional_warehouse_v2.csv", "edgeiq_sectional_quarantine_v2.csv", "edgeiq_sectional_warehouse_v2_summary.csv",
    "edgeiq_sectional_warehouse_v3.csv", "edgeiq_sectional_quarantine_v3.csv", "edgeiq_sectional_warehouse_v3_summary.csv",
    "edgeiq_sectional_pipeline_v2_audit.csv", "edgeiq_sectional_pipeline_v2_summary.csv", "edgeiq_sectional_split_failure_reasons_v2.csv",
    "edgeiq_sectional_pipeline_audit.csv", "edgeiq_sectional_pipeline_summary.csv",
    "edgeiq_sectional_identity_recovery_v3_summary.csv", "edgeiq_sectional_identity_recovery_v3_files.csv", "edgeiq_sectional_identity_recovery_v3_samples.csv",
    "edgeiq_sectional_source_roles_v3.csv", "edgeiq_sectional_source_roles_v3_summary.csv",
}
...
def path_date_hint(path: Path) -> str:
    m = re.search(r"(?<!\d)(20\d{2})[-_]?([01]\d)[-_]?([0-3]\d)(?!\d)", str(path))
    return f"{m.group(1)}-{m.group(2)}-{m.group(3)}" if m else ""


def path_race_hint(path: Path) -> str:
    m = re.search(r"(?:^|[^a-z0-9])r(?:ace)?[_ -]?(\d{1,2})(?:[^0-9]|$)", str(path).lower())
...
def setup(conn: sqlite3.Connection) -> None:
    ...
    CREATE TABLE best (
      ...
      source_file TEXT, identity_method TEXT, validation_status TEXT, ingested_at TEXT
    );
...
def main() -> None:
    ...
    recovered_date = recovered_race_no = recovered_both = 0
        ...
        try:
            with path.open("r", newline="", encoding="utf-8-sig") as h:
                ...
                if not any(c.get(k) for k in ("last200","last400","last600")):
                    continue
                pd = path_date_hint(path)
                pr = path_race_hint(path)
                for row_no, row in enumerate(reader, 2):
                    seen += 1
                    race_date = get(row,c,"race_date")[:10]
                    track = norm_track(get(row,c,"track"))
                    race_no = norm_race_no(get(row,c,"race_no"))
                    identity_method = "DIRECT_COLUMNS"
                    used_date = used_race = False
                    if not race_date and pd:
                        race_date = pd; used_date = True
                    if not race_no and pr:
                        race_no = pr; used_race = True
                    if used_date and used_race:
                        identity_method = "PATH_DATE_AND_RACE_RECOVERY"; recovered_both += 1
                    elif used_date:
                        identity_method = "PATH_DATE_RECOVERY"; recovered_date += 1
                    elif used_race:
                        identity_method = "PATH_RACE_RECOVERY"; recovered_race_no += 1
                    ...
                    ON CONFLICT(canonical_key) DO UPDATE SET
                      ...
                      identity_method=excluded.identity_method, validation_status=excluded.validation_status,
                    ...
                    """, (canonical_key,race_date,track,race_no,get(row,c,"meeting_id"),get(row,c,"race_id"),horse_id,horse,hk,
                          ...
                          str(path.relative_to(ROOT)),identity_method,"CANONICAL_VALIDATED_NAMED_SPLITS",now))
                    if seen % 100000 == 0:
    ...
    recovered_rows = conn.execute("SELECT COUNT(*) FROM best WHERE identity_method <> 'DIRECT_COLUMNS'").fetchone()[0]
    ...
    summary_rows = [
        ...
        ("dedup_replacements", dedup_updates), ("identity_recovered_rows", recovered_rows),
        ("path_date_recoveries_attempted", recovered_date), ("path_race_recoveries_attempted", recovered_race_no),
        ("path_date_and_race_recoveries_attempted", recovered_both),
        ("checkpoint_directories_excluded", "YES"),
    ...
    print("="*90)
    print("EDGEIQ SECTIONAL WAREHOUSE V3")
    print("="*90)
