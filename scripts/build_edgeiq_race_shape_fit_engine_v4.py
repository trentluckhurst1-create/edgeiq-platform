...
V3_AUDIT_PATH = DATA_DIR / "edgeiq_race_shape_fit_v3_audit.csv"

OUTPUT_PATH = DATA_DIR / "edgeiq_race_shape_fit_v4.csv"
AUDIT_PATH = DATA_DIR / "edgeiq_race_shape_fit_v4_audit.csv"
...
def canonical_horse_key(value: str | None) -> str:
    raw = remove_bracketed_country(text(value).upper())
...
def runner_key(row: dict[str, str]) -> tuple[str, str, str, str]:
    return (
        ...
        canonical_horse_key(row.get("horse_key") or row.get("canonical_horse_key") or row.get("horse")),
    )
...
def build_dna_lookup(rows: list[dict[str, str]]) -> dict[str, dict[str, str]]:
    ...
    for row in rows:
        key = canonical_horse_key(row.get("canonical_horse_key") or row.get("horse_key") or row.get("horse"))
        if key:
            lookup[key] = row
    return lookup
...
def read_v3_audit() -> dict[str, str]:
    rows = read_csv(V3_AUDIT_PATH)
    return rows[0] if rows else {}


def build_race_shape_fit_v4() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    live_rows = read_csv(LIVE_FEED_PATH)
    dna_rows = read_csv(HORSE_DNA_PATH)
    tactical_rows = read_csv(TACTICAL_BUCKET_PATH)
    v3_audit = read_v3_audit()
    ...
    matched_dna_v3 = to_int(v3_audit.get("matched_dna_rows"))
    usable_fit_v3 = to_int(v3_audit.get("usable_fit_rows"))

    audit = {
        "built_at": datetime.now(timezone.utc).isoformat(),
        "live_terminal_rows_loaded": len(live_rows),
        "horse_dna_source": HORSE_DNA_PATH.name,
        "horse_dna_rows_loaded": len(dna_rows),
        "tactical_bucket_rows_loaded": len(tactical_rows),
        "output_rows": len(output_rows),
        "matched_DNA_rows_V3": matched_dna_v3,
        "matched_DNA_rows_V4": matched_dna,
        "usable_fit_rows_V3": usable_fit_v3,
        "usable_fit_rows_V4": usable_fit,
        "match_improvement": matched_dna - matched_dna_v3,
        "usable_fit_improvement": usable_fit - usable_fit_v3,
        "matched_dna_rows": matched_dna,
        "unmatched_dna_rows": unmatched_dna,
        "usable_fit_rows": usable_fit,
        "insufficient_rows": insufficient_fit,
        ...
        "tempo_rule": "Preserved V3 rule: FAST/HIGH only primary_pressure_count>=2 OR forward_count>=6 AND leader_count>=2; HONEST/MODERATE forward_count 3-5; CONTROLLED/LOW <=2.",
        "scoring_method": "Preserved V3 method: 50% tempo fit + 50% position fit, followed by position-conflict caps.",
        "matching_method": "Canonical horse identity match using edgeiq_horse_dna_v2.canonical_horse_key/horse_key and canonicalised current runner horse_key.",
        "status": "RACE_SHAPE_FIT_V4_BUILT" if live_rows and dna_rows and tactical_rows and output_rows else "RACE_SHAPE_FIT_V4_INSUFFICIENT_INPUTS",
    }
...
def main() -> None:
    output_rows, audit = build_race_shape_fit_v4()
    write_csv(OUTPUT_PATH, output_rows, OUTPUT_COLUMNS)
    write_csv(AUDIT_PATH, [audit], list(audit.keys()))

    print(f"Race shape fit V4 rows written: {len(output_rows)}")
    print(f"Audit written: {AUDIT_PATH}")
    print(f"Status: {audit['status']}")
    print(f"Matched DNA rows V3 -> V4: {audit['matched_DNA_rows_V3']} -> {audit['matched_DNA_rows_V4']}")
    print(f"Usable fit rows V3 -> V4: {audit['usable_fit_rows_V3']} -> {audit['usable_fit_rows_V4']}")
