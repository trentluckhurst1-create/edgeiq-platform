...
SRC = DATA / "edgeiq_current_field_projection_V6_1_RESEARCH_BACKFILL_FILTER_V3_20260617.csv"

OUT = DATA / "edgeiq_current_fair_prices_V6_1_RESEARCH_BACKFILL_FILTER_V3_20260617.csv"
SUMMARY = DATA / "edgeiq_current_fair_prices_V6_1_RESEARCH_BACKFILL_FILTER_V3_20260617_summary.csv"
...
def main():
    if not SRC.exists():
        raise FileNotFoundError(f"Missing projection candidate: {SRC}")
    ...
    fallback = out["backfill_filter_v3_action"].astype(str).eq("FALLBACK_TO_V5_2_BACKFILLED_ONLY") if "backfill_filter_v3_action" in out.columns else pd.Series(False, index=out.index)
    ...
    summary_rows = [
        ...
        {"section": "overall", "metric": "fallback_to_v5_2_backfilled_only_rows", "value": int(fallback.sum())},
    ...
    print("[FAIR_PRICE_BACKFILL_FILTER_V3] COMPLETE")
    print(f"source={SRC}")
