...
LEGACY_SNAPSHOT_REQUIRED_FIELDS = [
...
CURRENT_SNAPSHOT_REQUIRED_FIELDS = [
    "race_entry_horse_performance_snapshot_id", "canonical_race_id", "canonical_runner_id", "race_date",
...
TRACK_ALIASES = {
    "PICKLEBETPARKWODONGA": "WODONGA",
    "BET365PARKWODONGA": "WODONGA",
    "PARKWODONGA": "WODONGA",
    "WODONGA": "WODONGA",
    "PICKLEBETPARKWERRIBEE": "WERRIBEE",
    "WERRIBEE": "WERRIBEE",
}


def clean_track(value: object) -> str:
    compact = re.sub(r"[^A-Z0-9]", "", upper_text(value))
    if compact in TRACK_ALIASES:
        return TRACK_ALIASES[compact]
    without_sponsor = re.sub(
        r"^(SPORTSBET|LADBROKES|BET365|PICKLEBETPARK|PICKLEBET|SOUTHSIDE)",
        "",
        compact,
    )
    without_sponsor = without_sponsor.removeprefix("PARK")
    return TRACK_ALIASES.get(without_sponsor, without_sponsor or compact)
...
def positive_integer(value: object, field_name: str) -> int:
    ...
    if parsed < 0 or (parsed == 0 and field_name != "rating_age_days"):
...
def main() -> None:
    ...
    is_legacy_snapshot_schema = all(field in snapshot_fields for field in LEGACY_SNAPSHOT_REQUIRED_FIELDS)
    is_current_snapshot_schema = all(field in snapshot_fields for field in CURRENT_SNAPSHOT_REQUIRED_FIELDS)
    if not is_legacy_snapshot_schema and not is_current_snapshot_schema:
        fail(f"{SNAPSHOT_PATH.name} missing a supported snapshot schema. fields={snapshot_fields}")
    ...
    for snapshot in snapshot_rows:
        ...
        race_entry_id = text(snapshot.get("race_entry_id", ""))
        race_id = text(snapshot.get("race_id", "")) or required_text(snapshot, "canonical_race_id", snapshot_id)
        runner_id = text(snapshot.get("runner_id", "")) or required_text(snapshot, "canonical_runner_id", snapshot_id)
        entry = (race_entries_by_id.get(race_entry_id) if race_entry_id else None) or race_entries_by_pair.get((race_id, runner_id))
        if entry is None:
            fail(f"{snapshot_id}: missing governed race entry {race_entry_id or 'DERIVED_BY_CANONICAL_PAIR'} for {race_id} / {runner_id}.")
        if not race_entry_id:
            race_entry_id = required_text(entry, "race_entry_id", f"{snapshot_id}:current-race-entry-adapter")
        ...
        if selected_rating_date > race_date:
            fail(f"{snapshot_id}: selected rating date is after race date.")
