...
def main() -> None:
    ...
    canonical_field_count = delta_fields.count("winner_canonical_horse_id")
    canonical_field_index = delta_fields.index("winner_canonical_horse_id") if canonical_field_count == 1 else -1
    winner_name_index = delta_fields.index("winner_horse_name") if "winner_horse_name" in delta_fields else -1
    check(
        "winner_canonical_horse_id_field_order",
        canonical_field_count == 1 and canonical_field_index + 1 == winner_name_index,
        {
            "field_count": canonical_field_count,
    ...
    if delta_mode == "RECOVERED_CANONICAL":
        ...
        for row in delta_rows:
            ...
            source_horse_id = text(timing.get("winner_canonical_horse_id"))
            output_horse_id = text(row.get("winner_canonical_horse_id"))
            if source_horse_id and not output_horse_id:
                canonical_identity_blanks.append(observation_id)
            if output_horse_id != source_horse_id:
                canonical_identity_errors.append(observation_id)
        ...
        horse_ids_by_observation: dict[str, set[str]] = {}
        for row in delta_rows:
            observation_id = text(row.get("benchmark_observation_id"))
            if observation_id:
                horse_ids_by_observation.setdefault(observation_id, set()).add(text(row.get("winner_canonical_horse_id")))
        canonical_identity_conflicts = sorted(observation_id for observation_id, values in horse_ids_by_observation.items() if len(values) > 1)
    else:
