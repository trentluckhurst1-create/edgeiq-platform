...
OUT = DATA / "edgeiq_condition_pars_v5_1.csv"
AUDIT = DATA / "edgeiq_condition_pars_v5_1_audit.csv"
...
def clean_condition(row: pd.Series) -> str:
    condition = str(row.get("condition_recovered", "")).upper().strip()
    token = str(row.get("condition_token_recovered", "")).upper().strip()
    blob = f"{condition} {token}"
...
def audit_row(section: str, metric: str, value: object = "", condition_group: str = "", notes: str = "") -> dict[str, object]:
    return {
        "section": section,
        "metric": metric,
        "condition_group_v5_1": condition_group,
        "value": value,
        "notes": notes,
    }
...
def main() -> None:
    print("=" * 90)
    print("EDGEIQ CONDITION PARS V5.1 - ADJUSTMENT MODE")
    print("=" * 90)
    ...
    df["condition_group_v5_1"] = df.apply(clean_condition, axis=1)
    ...
    for condition, group in df.groupby("condition_group_v5_1", dropna=False):
        ...
        rows.append(
            {
                "condition_group_v5_1": condition,
                "run_count": run_count,
                ...
                "condition_absolute_par_rating_v5_1": fmt(absolute_par),
                "condition_global_baseline_v5_1": fmt(global_baseline),
                "condition_par_rating_v5_1": fmt(adjustment),
                "condition_par_method_v5_1": method,
                "condition_par_confidence_v5_1": confidence,
                "condition_par_mode_v5_1": "ADJUSTMENT_FROM_GLOBAL_BASELINE",
                "condition_global_method_v5_1": global_method,
                "built_at": built_at,
            }
        )

    out = pd.DataFrame(rows)
    out["sort_order"] = out["condition_group_v5_1"].map(CONDITION_ORDER).fillna(99)
    ...
    audit_rows = [
        ...
        audit_row("overall", "condition_par_rows", len(out)),
    ...
    for row in out.to_dict("records"):
        audit_rows.append(
            audit_row(
                "condition_pars",
                "condition_adjustment_v5_1",
                row["condition_par_rating_v5_1"],
                condition_group=row["condition_group_v5_1"],
                notes=f"absolute={row['condition_absolute_par_rating_v5_1']}; baseline={row['condition_global_baseline_v5_1']}; method={row['condition_par_method_v5_1']}; confidence={row['condition_par_confidence_v5_1']}",
            )
        )
