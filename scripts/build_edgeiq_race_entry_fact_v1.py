...
ACCEPTED_CURRENT_RACE_STATUSES = {
    "",
    "FINALFIELDS",
    "OPEN",
}
...
def main():
    ...
    for meeting in obj.get('meetings',[]) or []:
        ...
        for race in meeting.get('races',[]) or []:
            ...
            race_status_key = re.sub(
                r"[^A-Z0-9]+",
                "",
                race_status.upper(),
            )
            if d is None or mdate not in TARGET_DATE_SET:
                race_reject='STALE_RACE_DATE'
            elif race_status_key not in ACCEPTED_CURRENT_RACE_STATUSES:
