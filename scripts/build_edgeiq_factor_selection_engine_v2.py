...
OUT = DATA / "edgeiq_factor_selection_engine_v2.csv"
SUMMARY = DATA / "edgeiq_factor_selection_engine_v2_summary.csv"
...
if factor_rows.empty:
    ...
    for race_key, g in factor_rows.groupby(["race_date", "track", "race_no"], dropna=False):
        ...
        fg["importance_score"] = fg["strength"] + fg["runner_count"]
        fg = fg.sort_values(["importance_score", "runner_count"], ascending=[False, False]).head(3)
        ...
        while len(top) < 3:
            top.append({"factor_name": "", "direction": "", "importance_score": 0, "runners": ""})
...
summary = pd.DataFrame([{
    "status": "FACTOR_SELECTION_ENGINE_V2_BUILT",
    "race_rows": len(out),
