...
def main():
    ...
    if PROB.exists():
        prob = pd.read_csv(PROB, dtype=str).fillna("")
        if {"track","race_no","horse","v3_probability","v3_fair_price"}.issubset(prob.columns):
            prob["horse_canon"] = prob["horse"].map(canon)
            prob["track_join"] = prob["track"].map(canon)
            prob["race_no_join"] = prob["race_no"].map(clean)

            prob_small = prob[
                ["track_join","race_no_join","horse_canon","v3_probability","v3_fair_price","v3_realism_grade","v3_pricing_action"]
            ...
            df["fair_price"] = df["v3_fair_price"].fillna("")
            df["v3_probability"] = df["v3_probability_prob"].fillna("") if "v3_probability_prob" in df.columns else df["v3_probability"].fillna("")
            df["v3_realism_grade"] = df["v3_realism_grade_prob"].fillna("") if "v3_realism_grade_prob" in df.columns else df["v3_realism_grade"].fillna("")
            df["v3_pricing_action"] = df["v3_pricing_action_prob"].fillna("") if "v3_pricing_action_prob" in df.columns else df["v3_pricing_action"].fillna("")
