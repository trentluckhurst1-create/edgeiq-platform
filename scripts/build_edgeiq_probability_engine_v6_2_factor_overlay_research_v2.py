...
OUT = DATA / "edgeiq_probability_engine_v6_2_factor_overlay_research_v2.csv"
SUMMARY = DATA / "edgeiq_probability_engine_v6_2_factor_overlay_research_v2_summary.csv"
BY_FACTOR = DATA / "edgeiq_probability_engine_v6_2_factor_overlay_research_v2_by_factor.csv"
BY_RACE = DATA / "edgeiq_probability_engine_v6_2_factor_overlay_research_v2_by_race.csv"
...
for _, r in dna.iterrows():
    ...
    row = r.to_dict()
    row["factor_overlay_score_v6_2_v2"] = round(overlay, 3)
    row["factor_adjustment_pct_v6_2_v2"] = adjustment
    row["positive_factor_total_v6_2_v2"] = pos
    row["negative_factor_total_v6_2_v2"] = neg
    row["base_probability"] = "" if pd.isna(base_prob) else round(base_prob, 8)
    row["research_probability_v6_2_v2"] = "" if pd.isna(research_prob) else round(research_prob, 8)
    row["research_fair_price_v6_2_v2"] = "" if pd.isna(research_fair) else round(research_fair, 4)
...
out["research_fair_price_v6_2_v2_num"] = out["research_fair_price_v6_2_v2"].apply(num)

out["base_rank"] = out.groupby(["race_date", "track", "race_no"])["fair_price_num"].rank(method="min", ascending=True)
out["v6_2_v2_rank"] = out.groupby(["race_date", "track", "race_no"])["research_fair_price_v6_2_v2_num"].rank(method="min", ascending=True)
...
for keys, g in out.groupby(["race_date", "track", "race_no"], dropna=False):
    ...
    v2_top = priced.sort_values("research_fair_price_v6_2_v2_num").iloc[0]
    ...
    race_rows.append({
        ...
        "v6_2_v2_top": v2_top["horse"],
        "base_top_fair": base_top["fair_price"],
        "v6_2_v2_top_fair": v2_top["research_fair_price_v6_2_v2"],
...
priced["fair_delta_pct"] = np.where(
    ...
    ((priced["research_fair_price_v6_2_v2_num"] - priced["fair_price_num"]) / priced["fair_price_num"]) * 100,
    np.nan,
)
...
summary = pd.DataFrame([
    ["status", "PROBABILITY_ENGINE_V6_2_FACTOR_OVERLAY_RESEARCH_V2_BUILT"],
    ["rows", len(out)],
    ...
    ["positive_factor_total", int(out["positive_factor_total_v6_2_v2"].sum())],
    ["negative_factor_total", int(out["negative_factor_total_v6_2_v2"].sum())],
    ["avg_overlay_score", round(out["factor_overlay_score_v6_2_v2"].mean(), 3)],
    ["avg_adjustment_pct", round(out["factor_adjustment_pct_v6_2_v2"].mean(), 3)],
...
print("[PROBABILITY_ENGINE_V6_2_FACTOR_OVERLAY_RESEARCH_V2] COMPLETE")
