...
OUT = DATA / "edgeiq_current_field_projection_v5_2_research_replay.csv"
SUMMARY = DATA / "edgeiq_current_field_projection_v5_2_research_replay_summary.csv"
MIGRATION = DATA / "edgeiq_current_field_projection_v5_2_research_band_migration.csv"
...
def main():
    ...
    hist["research_rating"] = pd.to_numeric(hist["performance_rating_v5_1_research"], errors="coerce")
    ...
    for _, r in curr.iterrows():
        ...
        out["projected_rating_research"] = round(projected, 2) if pd.notna(projected) else ""
        out["projection_gap_research"] = round(gap, 2) if pd.notna(gap) else ""
        out["projection_band_research"] = band(gap)
    ...
    for col in ["projection_band_v5_2", "projection_band_research"]:
    ...
    new_gaps = pd.to_numeric(out_df["projection_gap_research"], errors="coerce")
    ...
    mig = (
        out_df
        .groupby(["projection_band_v5_2", "projection_band_research"], dropna=False)
        .size()
