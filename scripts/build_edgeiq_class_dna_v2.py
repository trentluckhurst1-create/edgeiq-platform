...
OUT_ALL = DATA / "edgeiq_class_dna_v2.csv"
OUT_LIVE = DATA / "edgeiq_live_class_dna_v2.csv"
OUT_SUMMARY = DATA / "edgeiq_class_dna_v2_summary.csv"
OUT_JSON = DATA / "edgeiq_class_dna_v2_summary.json"
...
def class_group(x):
    ...
    if s in ["MAIDEN", "MDN"] or "MAIDEN" in s or s == "MDN":
        return "MDN"
...
hist["horse"] = hist.get("horseName", "").apply(canon_horse)
...
exact = hist.groupby(["horse", "class_group"], dropna=False).agg(
...
exact["exact_class_fit_band"] = exact.apply(lambda r: score_band(r["exact_class_fit_score"], r["exact_class_starts"]), axis=1)

career = hist.groupby("horse").agg(
...
career["career_class_band"] = career.apply(lambda r: score_band(r["career_class_score"], r["career_class_starts"]), axis=1)

exact = exact.merge(career, on="horse", how="left")
...
for c in [
    ...
    "career_best_class_level","career_avg_class_level","career_min_class_level"
]:
...
summary_rows = [
    {"metric": "status", "value": "CLASS_DNA_V2_BUILT"},
    ...
    {"metric": "unique_horses", "value": career["horse"].nunique()},
