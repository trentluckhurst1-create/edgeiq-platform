...
def main():
    if not CATALOG.exists(): raise SystemExit('missing current three-day catalog')
    catalog=json.loads(CATALOG.read_text(encoding='utf-8-sig'))
    rating_rows=rows(RATINGS); epi_rows=rows(OLD_EPI)
    ratings={}
    for r in rating_rows:
        hid=clean(r.get('canonical_horse_id')); d=clean(r.get('rating_as_of_date'))
        if not hid:continue
        ratings.setdefault(hid,[]).append(r)
    ...
    for m in catalog.get('meetings',[]) or []:
        ...
        for race in m.get('races',[]) or []:
            ...
            for node in race.get('runners',[]) or []:
                ...
                epi_value=clean(er.get('epi_value')) if er else ''
                epi_rank=clean(er.get('epi_rank')) if er else ''
                epi_status=clean(er.get('epi_status')) if er else 'EPI_UNAVAILABLE'
                out.append({
                    'race_intelligence_feed_id':f'LIVE|{date}|{norm(track)}|R{rn}|{runner_id or norm(name)}',
                    'canonical_race_id':race_id,'canonical_runner_id':runner_id,'canonical_horse_name':name,'race_date':date,
                    'canonical_track':track,'race_number':rn,'barrier':clean(source.get('barrierNumber') or official.get('barrier')),
                    'weight_kg':clean(source.get('weight') or official.get('weight')),'jockey_name':clean(source.get('jockeyName') or official.get('jockey')),
                    'trainer_name':clean(source.get('trainerName') or official.get('trainer')),'entry_participation_status':'SCRATCHED' if source.get('scratched') else 'ACTIVE_ENTRY',
                    'historical_rating_value':hist,'suitability_composite_delta':'','projected_performance_value':'','epi_value':epi_value,'epi_rank':epi_rank,
                    'historical_rating_status':hist_status,'suitability_status':'SUITABILITY_UNAVAILABLE','projected_performance_status':'PROJECTED_PERFORMANCE_UNAVAILABLE',
                    'epi_status':epi_status,'race_intelligence_status':'AVAILABLE_PARTIAL' if hist or epi_value else 'RACE_INTELLIGENCE_UNAVAILABLE',
                    'race_intelligence_reason_code':'CURRENT_DAY_PRIOR_ONLY','source_race_context_evidence_sha256':'','source_projected_performance_evidence_sha256':'','source_epi_evidence_sha256':'',
                    'race_intelligence_builder_version':'EDGEIQ_CURRENT_RACE_INTELLIGENCE_V3','race_intelligence_method_version':'STRICT_PRIOR_CURRENT_RUNNER_JOIN_V1','race_intelligence_evidence_sha256':''})
    ...
    print(f'CURRENT_RACE_INTELLIGENCE_ROWS={len(out)} ERR_AVAILABLE={sum(bool(r["historical_rating_value"]) for r in out)} EPI_AVAILABLE={sum(bool(r["epi_value"]) for r in out)}')
