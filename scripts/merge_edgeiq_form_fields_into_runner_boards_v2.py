...
form = pd.read_csv(form_path, dtype=str, keep_default_na=False)
...
for path in targets:
    ...
    backup = path.with_name(path.stem + "_BACKUP_BEFORE_FORM_FIELDS_MERGE_V2_20260628" + path.suffix)
    df.to_csv(backup, index=False)

    # create all form columns as object/string first
    for c in form_cols:
...
summary_out = DATA / "edgeiq_form_fields_merged_into_runner_boards_v2_summary.csv"
report_out = DATA / "edgeiq_form_fields_merged_into_runner_boards_v2_report.txt"

summary.to_csv(summary_out, index=False)

with open(report_out, "w", encoding="utf-8") as f:
    f.write("[EDGEIQ_FORM_FIELDS_MERGED_INTO_RUNNER_BOARDS_V2]\n")
    f.write(f"built_at={datetime.now(timezone.utc).isoformat()}\n\n")
    f.write(summary.to_string(index=False))
    f.write("\n\npricing_maths_changed=NO\nv6_1_changed=NO\nv7_2g2_changed=NO\n")

print("[EDGEIQ_FORM_FIELDS_MERGED_INTO_RUNNER_BOARDS_V2] COMPLETE")
