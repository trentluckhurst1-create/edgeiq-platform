...
async function capture(name, selector = ".eiq-map-v1", fullPage = true) {
  const file = path.join(outDir, `edgeiq_map_engineering_v1_${name}.png`);
...
try {
  ...
  await clickFirst("Map tab", [
    page.getByRole("tab", { name: /^MAP$/i }),
    page.getByRole("button", { name: /^MAP$/i }),
  ]);

  await capture("01_race_map");
  await capture("02_map_visual", ".eiq-map-v1-visual", false);
  await capture("03_map_table", ".eiq-map-v1-table-scroll", false);

  await page.setViewportSize({ width: 1180, height: 900 });
  await page.waitForTimeout(600);
  await capture("04_narrow_desktop");

  await fs.writeFile(
    path.join(outDir, "edgeiq_map_engineering_v1_browser_notes.txt"),
    [`URL: ${baseUrl}`, "", "Console / capture notes:", ...logs].join("\n"),
    "utf8",
  );

  console.log("EDGEIQ_MAP_ENGINEERING_V1_SCREENSHOTS_WRITTEN");
} finally {
  await browser.close();
}
