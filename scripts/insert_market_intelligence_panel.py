...
old_overround = '''  const favourite = rows.find((row) => row.live !== null) ?? null;
...
new_overround = '''  const activeRows = rows.filter((row) => !row.isScratched);
...
if old_overround not in text:
    raise SystemExit("Could not find overround block. No changes made.")

text = text.replace(old_overround, new_overround)
...
if "market-intel-panel" not in text:
    if insert_before not in text:
        raise SystemExit("Could not find panel insert point. No changes made.")
    text = text.replace(insert_before, panel + "\n" + insert_before)

path.write_text(text, encoding="utf-8")
print("MARKET INTELLIGENCE PANEL INSERTED")
