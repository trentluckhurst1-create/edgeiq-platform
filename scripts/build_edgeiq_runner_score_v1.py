...
OUT_FILE = DATA / "edgeiq_runner_score_v1.csv"
AUDIT_FILE = DATA / "edgeiq_runner_score_v1_audit.csv"
...
def reason(row):
    ...
    if num(row.get("runner_score_v1"), 0) >= 85:
        parts.append("elite race-relative profile")
    elif num(row.get("runner_score_v1"), 0) >= 70:
        parts.append("positive race-relative profile")

    if num(row.get("strength_adjusted_rating_v2"), 0) >= 58:
        parts.append("strong strength rating")
...
def main():
    ...
    strength = pd.to_numeric(out.get("strength_adjusted_rating_v2"), errors="coerce").fillna(50)
    ...
    out["runner_score_v1"] = out.groupby("race_key_join")["runner_raw_score_v1"].transform(race_scale).round(3)

    out["runner_rank_v1"] = out.groupby("race_key_join")["runner_score_v1"].rank(
        ascending=False,
        method="min"
    ).astype(int)

    out["runner_percentile_v1"] = (
        out.groupby("race_key_join")["runner_score_v1"].rank(pct=True, method="max") * 100
    ).round(3)

    out["runner_band_v1"] = out["runner_score_v1"].map(runner_band)

    conf = pd.Series(35.0, index=out.index)
    conf += np.where(pd.to_numeric(out.get("strength_adjusted_rating_v2"), errors="coerce").notna(), 15, 0)
    ...
    conf += np.where(pd.to_numeric(out.get("pace_advantage_score_v4"), errors="coerce").notna(), 8, 0)
    conf += np.where(pd.to_numeric(out.get("race_shape_fit_score_v1"), errors="coerce").notna(), 8, 0)
    conf += np.where(pd.to_numeric(out.get("hidden_runner_score_v2"), errors="coerce").notna(), 4, 0)
    conf += np.where(pd.to_numeric(out.get("vulnerability_score_v1"), errors="coerce").notna(), 4, 0)

    conf -= np.where(out["runner_score_v1"] < 25, 5, 0)
    conf -= np.where(pd.to_numeric(out.get("vulnerability_score_v1"), errors="coerce").fillna(0) >= 75, 5, 0)

    out["runner_confidence_score_v1"] = conf.clip(0, 90).round(1)
    out["runner_confidence_band_v1"] = out["runner_confidence_score_v1"].map(confidence_band)

    out["runner_reason_v1"] = out.apply(reason, axis=1)

    out["runner_score_engine"] = "RUNNER_SCORE_V1"
    out["runner_score_inputs"] = "strength_v2|pace_advantage_v4|shape_fit_v1|hidden_v2|predictability_v1|vulnerability_v1"
    ...
    audit = pd.DataFrame([{
        ...
        "avg_runner_score": round(float(out["runner_score_v1"].mean()), 3),
        "min_runner_score": round(float(out["runner_score_v1"].min()), 3),
        "max_runner_score": round(float(out["runner_score_v1"].max()), 3),
        "elite": int((out["runner_band_v1"] == "ELITE").sum()),
        "strong": int((out["runner_band_v1"] == "STRONG").sum()),
        "positive": int((out["runner_band_v1"] == "POSITIVE").sum()),
        "neutral": int((out["runner_band_v1"] == "NEUTRAL").sum()),
        "poor": int((out["runner_band_v1"] == "POOR").sum()),
        "output": str(OUT_FILE),
    }])
    ...
    print("[RUNNER_SCORE_V1] COMPLETE")
    print(f"rows={len(out)}")
    print(f"races={out['race_key_join'].nunique()}")
    print(f"wrote={OUT_FILE}")
    print(f"audit={AUDIT_FILE}")
    print(out["runner_band_v1"].value_counts(dropna=False).to_string())
