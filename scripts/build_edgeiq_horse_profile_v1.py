...
def main():
    ...
    for hk, g in ratings.sort_values("race_date_dt", ascending=False).groupby("horse_key", dropna=False):
        ...
        valid_rating_all = g[g["rating"].notna()].copy()`n        valid_rating = valid_rating_all[~valid_rating_all["is_backfilled"]].copy()
        ...
        rows.append({
            ...
            "profile_rating_source": "GENUINE_ONLY" if len(valid_rating) else "NO_GENUINE_RATING_BACKFILLED_ONLY",`n            "profile_source": "edgeiq_historical_performance_rating_v6_1_research.csv",
