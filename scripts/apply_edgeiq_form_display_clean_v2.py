...
OUT=os.path.join(DATA,'edgeiq_form_display_clean_v2_apply.csv')
SUM=os.path.join(DATA,'edgeiq_form_display_clean_v2_apply_summary.csv')
REP=os.path.join(DATA,'edgeiq_form_display_clean_v2_apply_report.txt')
...
update_cols=[c for c in form_cols if c.endswith('_v2') or c.startswith('form_') or c.startswith('last_start_') or c.endswith('_profile_display') or c in {'rating_trend','rating_trend_delta'}]
results=[]; summary=[]
for board in BOARDS:
    rows, cols=read_csv(board)
    backup=board.replace('.csv','_DISPLAY_CLEAN_V2_BACKUP_20260628.csv')
    if not os.path.exists(backup): shutil.copyfile(board,backup)
    out_cols=list(cols)
    for c in update_cols:
        if c not in out_cols: out_cols.append(c)
    matched=0; missing=0; out=[]
    for r in rows:
        ...
        out.append({c:r.get(c,'') for c in out_cols})
    ...
    rec={'board':os.path.basename(board),'rows':len(out),'races':races,'matched_form_rows':matched,'missing_form_rows':missing,'backup':os.path.basename(backup),'pricing_maths_changed':'NO','v6_1_changed':'NO','v7_2g2_changed':'NO'}
    results.append(rec)
...
rep=['EDGEiQ FORM DISPLAY CLEAN V2 APPLY']
for r in results: rep.append(f"{r['board']}: rows={r['rows']} races={r['races']} matched_form_rows={r['matched_form_rows']} missing_form_rows={r['missing_form_rows']}")
rep += ['pricing_maths_changed=NO','v6_1_changed=NO','v7_2g2_changed=NO','status=FORM_DISPLAY_CLEAN_V2_APPLIED']
