...
OUT = DATA / "edgeiq_runner_profile_engine_v1_2.csv"
SUMMARY = DATA / "edgeiq_runner_profile_engine_v1_2_summary.csv"
...
def derived_totals(cr):
    if cr.empty:
        return 0, 0, 0, "", ""

    x = cr.copy()

    for col in ["starts", "wins", "places"]:
        if col not in x.columns:
            x[col] = "0"

    x["_starts_num"] = x["starts"].map(to_num)
    x["_wins_num"] = x["wins"].map(to_num)
    x["_places_num"] = x["places"].map(to_num)

    totals = (
        x.groupby("context_type")[["_starts_num", "_wins_num", "_places_num"]]
        .sum()
        .reset_index()
    )
    ...
    best = totals.sort_values("_starts_num", ascending=False).iloc[0]

    starts = int(best["_starts_num"])
    wins = int(best["_wins_num"])
    places = int(best["_places_num"])

    win_pct = round((wins / starts) * 100, 1) if starts else ""
    place_pct = round((places / starts) * 100, 1) if starts else ""

    return starts, wins, places, win_pct, place_pct

print("[RUNNER_PROFILE_ENGINE_V1_2] START")
...
for _, r in live.iterrows():
    ...
    starts, wins, places, win_pct, place_pct = derived_totals(cr)
    ...
    if starts:
        profile_summary = f"{horse} has a {starts}-start EDGEiQ profile with {wins} wins and {places} placings."
    else:
        profile_summary = f"{horse} has limited historical profile data available."
...
summary = pd.DataFrame([{
    "status": "RUNNER_PROFILE_ENGINE_V1_2_BUILT",
    "rows": len(out),
    ...
    "none_profile_strength": int((out["profile_strength"] == "NONE").sum()),
...
print("[RUNNER_PROFILE_ENGINE_V1_2] COMPLETE")
