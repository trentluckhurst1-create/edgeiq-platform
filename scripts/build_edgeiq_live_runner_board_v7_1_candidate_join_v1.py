...
OUT_CANDIDATE = DATA / "edgeiq_live_runner_board_v7_1_candidate.csv"
OUT_AUDIT = DATA / "edgeiq_live_runner_board_v7_1_candidate_join_audit_v1.csv"
OUT_SUMMARY = DATA / "edgeiq_live_runner_board_v7_1_candidate_join_summary_v1.csv"
OUT_REPORT = DATA / "edgeiq_live_runner_board_v7_1_candidate_join_report_v1.txt"
...
def pick_live_source() -> Path | None:
...
def build_join_key(df: pd.DataFrame, prefix: str) -> tuple[pd.Series, list[str]]:
    ...
    key = date_values + "|" + track_values + "|" + race_values + "|" + horse_values
    return key.rename(f"{prefix}_join_key"), missing
...
def write_blocked_outputs(status: str, live_path: Path | None, live_df: pd.DataFrame, missing_reasons: list[str]) -> pd.DataFrame:
    ...
    summary_metrics = {
        ...
        "live_source_file_used": str(live_path.relative_to(BASE)) if live_path else "MISSING",
        ...
        "old_fair_price_column_found": "YES" if "fair_price" in live_df.columns else "NO",
        "old_live_price_column_found": "YES" if "live_price" in live_df.columns else "NO",
    ...
    report = build_report(summary_metrics, missing_reasons)
...
def build_report(metrics: dict[str, object], warnings: list[str]) -> str:
    ...
    return f"""EDGEiQ LIVE RUNNER BOARD V7.1 CANDIDATE JOIN V1
...
2. Join result
- Live source file used: {metrics.get('live_source_file_used')}.
- Live rows: {metrics.get('live_rows')}.
...
- Original live runner board files were not overwritten by this script.
- Terminal feeds were not modified by this script.
- UI files were not modified by this script.

4. Data quality notes
- Duplicate live keys: {metrics.get('duplicate_live_keys')}.
- Duplicate candidate keys: {metrics.get('duplicate_candidate_keys')}.
- Warnings:
{warning_text}

5. Review requirement
- Review required before any live wiring.
- Candidate may have low or zero match rate because the live board is current-day while V7/V7.1 artifacts are TAB-quality replay-history artifacts.
"""


def main() -> None:
    warnings = []
    live_path = pick_live_source()
    if live_path is None:
        summary = write_blocked_outputs(
            "CANDIDATE_JOIN_BLOCKED_SCHEMA_MISSING",
            None,
            pd.DataFrame(),
            ["No live runner board source file found."],
        )
        print(summary.to_string(index=False))
        print(OUT_REPORT.read_text(encoding="utf-8"))
        return

    live_df = read_csv_safe(live_path)
    ...
    live_key, live_missing = build_join_key(live_df, "live")
    ...
    if live_missing:
        schema_missing.append("Live source missing join fields: " + ", ".join(live_missing))
    if v71_missing:
    ...
    if schema_missing:
        summary = write_blocked_outputs("CANDIDATE_JOIN_BLOCKED_SCHEMA_MISSING", live_path, live_df, schema_missing)
        print(summary.to_string(index=False))
        print(OUT_REPORT.read_text(encoding="utf-8"))
        return

    live_work = live_df.copy()
    live_work["__join_key"] = live_key
    ...
    duplicate_live_keys = int(live_work["__join_key"].duplicated(keep=False).sum())
    ...
    v71_cols = ["__join_key"]
    for col in ["edgeiq_display_fair_price_v7_1", "display_price_engine", "artifact_status", "production_changed", "edgeiq_probability_v7", "edgeiq_fair_price_v7"]:
        if col in v71_work.columns:
            v71_cols.append(col)
    v71_map = v71_work[v71_cols].drop_duplicates("__join_key", keep="first")

    v7_cols = ["__join_key"]
    for col in ["edgeiq_probability_v7", "edgeiq_fair_price_v7", "probability_engine_version", "probability_engine_status"]:
        if col in v7_work.columns:
            v7_cols.append(col)
    v7_map = v7_work[v7_cols].drop_duplicates("__join_key", keep="first")

    merged = live_work.merge(v71_map, on="__join_key", how="left", suffixes=("", "_v71src"))
    merged = merged.merge(v7_map, on="__join_key", how="left", suffixes=("", "_v7src"))

    output = live_df.copy()
    probability_candidate = merged["edgeiq_probability_v7_v7src"] if "edgeiq_probability_v7_v7src" in merged.columns else merged.get("edgeiq_probability_v7", pd.Series([""] * len(merged)))
    fair_candidate = merged["edgeiq_fair_price_v7_v7src"] if "edgeiq_fair_price_v7_v7src" in merged.columns else merged.get("edgeiq_fair_price_v7", pd.Series([""] * len(merged)))
    display_candidate = merged["edgeiq_display_fair_price_v7_1"] if "edgeiq_display_fair_price_v7_1" in merged.columns else pd.Series([""] * len(merged))
    probability_version = merged["probability_engine_version"] if "probability_engine_version" in merged.columns else pd.Series(["V7_TEMPERATURE6_TAB_QUALITY"] * len(merged))
    display_version = merged["display_price_engine"] if "display_price_engine" in merged.columns else pd.Series(["V7_1_DISPLAY_FAIR_PRICE_LAYER"] * len(merged))

    output["edgeiq_probability_v7_candidate"] = probability_candidate.fillna("").astype(str)
    output["edgeiq_fair_price_v7_calibrated_candidate"] = fair_candidate.fillna("").astype(str)
    output["edgeiq_display_fair_price_v7_1_candidate"] = display_candidate.fillna("").astype(str)
    output["edgeiq_price_engine_version_candidate"] = probability_version.fillna("").astype(str)
    output["edgeiq_display_price_engine_version_candidate"] = display_version.fillna("").astype(str)

    matched_mask = (
        output["edgeiq_probability_v7_candidate"].str.strip().ne("")
        | output["edgeiq_display_fair_price_v7_1_candidate"].str.strip().ne("")
    )
    ...
    output["edgeiq_v7_1_join_key"] = live_work["__join_key"]
    ...
    if len(output) != len(live_df):
        status = "CANDIDATE_JOIN_BLOCKED_ROW_COUNT_CHANGED"
    elif matched_rows == 0:
        status = "CANDIDATE_JOIN_BUILT_WITH_ZERO_MATCHES_REVIEW_REQUIRED"
    else:
        status = "CANDIDATE_JOIN_BUILT_REVIEW_REQUIRED"

    output["edgeiq_v7_1_join_status"] = output["edgeiq_v7_1_join_status"].where(output["edgeiq_v7_1_join_status"] == "MATCHED", "UNMATCHED")
    output.to_csv(OUT_CANDIDATE, index=False)

    audit = pd.DataFrame({
        "live_source_file_used": str(live_path.relative_to(BASE)),
        "join_key": output["edgeiq_v7_1_join_key"],
        "race_date": output["race_date"] if "race_date" in output.columns else output["meeting_date"] if "meeting_date" in output.columns else "",
    ...
    summary_metrics = {
        ...
        "old_fair_price_column_found": "YES" if "fair_price" in live_df.columns else "NO",
        "old_live_price_column_found": "YES" if "live_price" in live_df.columns else "NO",
        "old_ui_fair_price_column_found": "YES" if "ui_fair_price" in live_df.columns else "NO",
        "old_win_pct_column_found": "YES" if "win_pct" in live_df.columns else "NO",
    ...
    if matched_rows == 0:
        warnings.append("Zero normalized-key matches. This can be expected when current-day live board rows do not exist in replay-history V7/V7.1 artifacts.")
    if duplicate_live_keys:
        warnings.append("Duplicate normalized live keys found; audit rows should be reviewed before any future wiring.")
