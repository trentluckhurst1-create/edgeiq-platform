...
DOCS=Path(os.environ.get('EDGEIQ_PERFORMANCE_DOCS_ROOT', ROOT/'docs'/'performance-intelligence')).resolve()
...
def verify():
    ...
    return p31,f
def snapshot_dir(snaps, phase):
    snap=snaps.get(phase) or {}
    sid=txt(snap.get('snapshot_id')) if isinstance(snap,dict) else ''
    portable=DOCS/'warehouse'/phase/sid
    if portable.exists(): return portable
    raw=Path(txt(snap.get('snapshot_path'))) if isinstance(snap,dict) else Path()
    if raw.exists(): return raw
    raise FileNotFoundError(f'Missing portable performance snapshot for {phase}: {portable}')
def current(cat):
    races=[]; runners=[]
...
def main():
    ...
    db=snapshot_dir(snaps,'phase2_7')/'edgeiq_performance_query_engine_v1.sqlite'; facts=DOCS/'warehouse'/'performance-facts-corrected'/snaps['phase1_7']['manifest']['source_snapshot_id']/'canonical_performance_facts_v0_2.csv'
