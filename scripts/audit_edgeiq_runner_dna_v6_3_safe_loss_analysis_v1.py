...
OUT = DATA / "edgeiq_runner_dna_v6_3_safe_loss_analysis_v1.csv"
SUMMARY = DATA / "edgeiq_runner_dna_v6_3_safe_loss_analysis_v1_summary.csv"
...
if len(out):
    ...
    factor_summary["pct_of_losses"] = (factor_summary["times_responsible"] / len(out) * 100).round(2)
else:
    factor_summary = pd.DataFrame(columns=["largest_factor_shift", "times_responsible", "pct_of_losses"])

header = pd.DataFrame([
    ["status", "RUNNER_DNA_V6_3_SAFE_LOSS_ANALYSIS_V1_BUILT"],
    ["version", VERSION],
    ["loss_rows", len(out)],
    ["source", SRC.name],
