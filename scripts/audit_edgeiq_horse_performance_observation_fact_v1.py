...
def main() -> None:
    ...
    if rating_rows and not CANONICAL_HORSE_IDENTITY.exists():
        check(
            "canonical_identity_registry_required_when_rating_rows_exist",
            False,
            "Governed canonical horse identity registry is missing.",
        )
        registry_rows = []
    elif CANONICAL_HORSE_IDENTITY.exists():
        _, registry_rows = read_csv(CANONICAL_HORSE_IDENTITY)

        check(
            "canonical_identity_registry_available",
            True,
            {"canonical_identity_registry_rows": len(registry_rows)},
        )
    else:
        registry_rows = []

        check(
            "canonical_identity_registry_not_required_for_empty_rating_base",
            not rating_rows,
            {
                "performance_rating_base_rows": len(rating_rows),
                "canonical_identity_registry_exists": False,
            },
        )
    ...
    governance_errors = [
        ...
        if (
            ...
            not in {
                ...
                "EXACT_CANONICAL_HORSE_ID_RATING_BASE",
            }
