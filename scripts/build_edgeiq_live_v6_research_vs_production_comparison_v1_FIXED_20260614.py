...
def action(edge, band):
    if edge is None or math.isnan(edge):
        return "NO_EDGE"

    good = {"ELITE","STRONG","POSITIVE","NEUTRAL"}
    ...
    if band in bad:
        return "PASS"
    if edge >= 18 and band in good:
        return "WATCH"
    if edge >= 10 and band in lean:
        return "LEAN"
    if edge <= -18:
        return "UNDERLAY"
    return "PASS"
...
def main():
    ...
    for _, r in df.iterrows():
        ...
        if not math.isnan(live_price) and not math.isnan(fair) and fair > 0:
            ...
            act = action(edge, r.get("projection_band_v6_research",""))
