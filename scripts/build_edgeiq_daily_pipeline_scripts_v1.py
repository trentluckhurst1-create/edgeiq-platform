...
STAGES = [
    ...
    "scripts/build_edgeiq_current_late_speed_v1.py", "scripts/complete_edgeiq_current_early_late_distance_aware_v3.py",
...
'''
write("scripts/run_edgeiq_daily_rolling_product_pipeline_v1.py", pipeline)

runner_ps = r'''param([switch]$RunOnce, [string]$Date)
...
exit $LASTEXITCODE
'''
write("scripts/automation/run_edgeiq_daily_pipeline.ps1", runner_ps)

installer_ps = r'''$ErrorActionPreference = "Stop"
$Repo = Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path)
...
'''
write("scripts/automation/install_edgeiq_daily_pipeline_task.ps1", installer_ps)
print("daily pipeline scripts written")
