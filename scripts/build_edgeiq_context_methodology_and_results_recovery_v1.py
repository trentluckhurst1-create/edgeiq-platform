...
def map_daily_fact_to_warehouse(row: dict[str, str]) -> dict[str, Any]:
    ...
    return {
        ...
        "has_results": "1",
        "has_speed_map": "",
        "runner_id": text(row.get("runner_source_id")),
        "race_entry_number": text(row.get("runner_source_id")),
        "horse": norm(row.get("runner_name")),
        "horse_code": text(row.get("runner_source_id")),
...
def repair_results_warehouse(apply_repair: bool) -> dict[str, Any]:
    ...
    current_daily = [r for r in daily if text(r.get("race_date")) >= "2026-07-20" and text(r.get("result_status")) == "RESULT_OFFICIAL"]
    before = len(warehouse)
    before_current = sum(1 for r in warehouse if text(r.get("race_date")) >= "2026-07-20")
    existing_keys = {
        (text(r.get("race_date")), token(r.get("track")), text(r.get("race_id")), text(r.get("runner_id")), norm(r.get("horse")))
        for r in warehouse
    }
    append_rows: list[dict[str, Any]] = []
    duplicate_rows = 0
    for fact in current_daily:
        mapped = map_daily_fact_to_warehouse(fact)
        key = (text(mapped.get("race_date")), token(mapped.get("track")), text(mapped.get("race_id")), text(mapped.get("runner_id")), norm(mapped.get("horse")))
        if key in existing_keys:
            duplicate_rows += 1
            continue
        existing_keys.add(key)
        append_rows.append(mapped)
    output_rows = warehouse + append_rows
    conflicts = 0
    rejections = 0
    if apply_repair and append_rows:
        all_fields = list(fields)
        for row in append_rows:
            for key in row:
                if key not in all_fields:
                    all_fields.append(key)
        write_csv(warehouse_path, output_rows, all_fields)
    summary = {
        "official_source": "edgeiq_daily_official_results_fact_v1 from retained Racing Australia public result evidence",
        "official_current_races": len({text(r.get("canonical_race_id")) for r in current_daily}),
        "official_current_runners": len(current_daily),
        "canonical_rows_before": before,
        "canonical_current_rows_before": before_current,
        "canonical_rows_after": len(output_rows) if apply_repair else before,
        "candidate_rows_to_append": len(append_rows),
        "duplicate_rows": duplicate_rows,
        "conflicting_rows": conflicts,
        "rejected_rows": rejections,
        "repair_implemented": "YES" if apply_repair and append_rows else ("ALREADY_PRESENT" if before_current > 0 else "NO"),
        "eight_row_discrepancy_classification": "RECOVERY_SOURCE_BYPASSED_CANONICAL_WAREHOUSE",
    }
    reconciliation = []
    for fact in current_daily:
        mapped = map_daily_fact_to_warehouse(fact)
        ...
        reconciliation.append({
            ...
            "horse": mapped["horse"],
            "canonical_before": "YES" if key in {
                (text(r.get("race_date")), token(r.get("track")), text(r.get("race_id")), text(r.get("runner_id")), norm(r.get("horse")))
                for r in warehouse
            } else "NO",
            "daily_fact_source": "public/data/edgeiq_daily_official_results_fact_v1.csv",
            "warehouse_action": "APPENDED" if any(mapped["race_id"] == a["race_id"] and mapped["runner_id"] == a["runner_id"] and mapped["horse"] == a["horse"] for a in append_rows) else "DUPLICATE_SKIPPED",
        })
    return {**summary, "reconciliation_rows": reconciliation}
